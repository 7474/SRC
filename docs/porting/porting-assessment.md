...
| **ユニットテスト** | 209ファイル, 3,841テストメソッド, 43,479行 | ✅ カバレッジ向上中 |
...
|---------------|----------------|-----------------|-------------------|-------------|
| VB互換関数 | 30 | 527 | 858 | ✅ 高 |
| 式評価（Expression） | 35 | 800 | 8,341 | ✅ 高 |
| ユニット（Unit） | 11 | 190 | 86,480 | ⚠️ 中 |
| コマンド（CmdData） | 21 | 411 | 21,172 | ⚠️ 中 |
| パイロット | 5 | 70 | 4,530 | ⚠️ 中 |
| イベント（Event） | 12 | 203 | 8,789 | ⚠️ 中 |
| モデル（Model） | 37 | 592 | 11,635 | ✅ 高 |
| マップ（Map） | 8 | 124 | 3,768 | ⚠️ 中 |
| 拡張（Extension） | 17 | 231 | - | ⚠️ 中 |
| ライブラリ（Lib） | 20 | 459 | 1,101 | ✅ 高 |
| **GUI（SRCSharpForm）** | **0** | **0** | **28,950** | **❌ テストなし** |

**現状**: GUI層（約29,000行）にユニットテストが存在しない。GUIは性質上テストが困難だが、コアロジックの大半（イベント・マップ・コマンド等）はテストで網羅されつつある。
...
- 2026-02-21: 進捗更新 — テストメソッド数 253 → 3,841、テストファイル数 31 → 209、カバレッジ分析テーブル更新
- 2026-02-20: 初版作成
