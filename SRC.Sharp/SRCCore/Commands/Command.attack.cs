...
namespace SRCCore.Commands
{
    public partial class Command
    {
        ...
        private void StartAttackCommand()
        {
            ...
            var partners = default(Unit[]);
            {
                ...
                if (withBlock.IsFeatureAvailable(argfname))
                {
                    ...
                    string argmidi_name = withBlock.FeatureData(argIndex1);
                ...
                string argattr7 = "Ｍ線";
                string argattr8 = "Ｍ移";
                if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr2))
                {
                    Map.AreaInCross(withBlock.x, withBlock.y, min_range, max_range);
                }
                else if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr3))
                {
                    Map.AreaInWideCross(withBlock.x, withBlock.y, min_range, max_range);
                }
                else if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr4))
                {
                    string argattr1 = "Ｍ扇";
                    Map.AreaInSectorCross(withBlock.x, withBlock.y, min_range, max_range, withBlock.WeaponLevel(SelectedWeapon, argattr1));
                }
                else if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr5) | withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr6) | withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr7))
                {
                    string arguparty1 = "すべて";
                    Map.AreaInRange(withBlock.x, withBlock.y, max_range, min_range, arguparty1);
                }
                else if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr8))
                {
                ...
                string argattr9 = "合";
                string argattr10 = "Ｍ";
                if (max_range == 1 & withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr9) & !withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr10))
                {
                    for (i = 1; i <= 4; i++)
                    {
                        ...
                        if (t is object)
                        {
                            if (withBlock.IsEnemy(t))
                            {
                                ...
                                withBlock.CombinationPartner(argctype_Renamed, SelectedWeapon, partners, t.x, t.y);
                ...
                string argattr15 = "Ｍ投";
                string argattr16 = "Ｍ線";
                if (!withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr15) & !withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr16))
                {
                    ...
                    for (i = GeneralLib.MaxLng(withBlock.x - max_range, 1); i <= loopTo1; i++)
                    {
                        ...
                        for (j = GeneralLib.MaxLng(withBlock.y - max_range, 1); j <= loopTo2; j++)
                        {
                            ...
                            // 武器の地形適応が有効？
                            if (withBlock.WeaponAdaption(SelectedWeapon, t.Area) == 0d)
                            {
                            ...
                            // 封印武器の対象属性外でない？
                            string argattr11 = "封";
                            if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr11))
                            {
                                if (withBlock.Weapon(SelectedWeapon).Power > 0 & withBlock.Damage(SelectedWeapon, t, true) == 0 | withBlock.CriticalProbability(SelectedWeapon, t) == 0)
                                {
                                    Map.MaskData[i, j] = true;
                                    goto NextLoop;
                                }
                            ...
                            // 限定武器の対象属性外でない？
                            string argattr12 = "限";
                            if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr12))
                            {
                                if (withBlock.Weapon(SelectedWeapon).Power > 0 & withBlock.Damage(SelectedWeapon, t, true) == 0 | withBlock.Weapon(SelectedWeapon).Power == 0 & withBlock.CriticalProbability(SelectedWeapon, t) == 0)
                                {
                                    Map.MaskData[i, j] = true;
                                    goto NextLoop;
                                }
                            ...
                            if (withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr13) | withBlock.IsUnderSpecialPowerEffect(argsptype))
                            {
                            ...
                            // ステルス＆隠れ身チェック
                            string argattr14 = "Ｍ";
                            if (!withBlock.IsWeaponClassifiedAs(SelectedWeapon, argattr14))
                            {
                                if (!withBlock.IsTargetWithinRange(SelectedWeapon, t))
                                {
                                    Map.MaskData[i, j] = true;
                                    goto NextLoop;
                                }
        ...
        private void FinishAttackCommand()
        {
            ...
            is_p_weapon = SelectedUnit.IsWeaponClassifiedAs(SelectedWeapon, argattr);
            // ADD END MARGE
            ...
            int argmax_prob = 0;
            int argmax_dmg = 0;
            SelectedTWeapon = COM.SelectWeapon(SelectedTarget, SelectedUnit, argamode, max_prob: argmax_prob, max_dmg: argmax_dmg);
            string argattr2 = "間";
            if (SelectedUnit.IsWeaponClassifiedAs(SelectedWeapon, argattr2))
            {
                SelectedTWeapon = 0;
            }
            ...
            // 攻撃の実施
            {
                ...
                if (withBlock7.Status_Renamed == "出撃" & withBlock7.MaxAction(true) > 0 & !withBlock7.IsConditionSatisfied(argIndex10) & SelectedTarget.Status_Renamed == "出撃")
                {
                    // まだ武器は使用可能か？
                    ...
                    else if (CommandState == "移動後ターゲット選択")
                    {
                        ...
                        if (!withBlock7.IsWeaponAvailable(SelectedWeapon, argref_mode3))
                        {
                            SelectedWeapon = -1;
                        }
                    }
                    else
                    {
                        ...
                        if (!withBlock7.IsWeaponAvailable(SelectedWeapon, argref_mode2))
                        {
                            SelectedWeapon = -1;
                        }
                    }
                    ...
                    if (SelectedWeapon > 0)
                    {
                        if (!withBlock7.IsTargetWithinRange(SelectedWeapon, SelectedTarget))
                        {
                            SelectedWeapon = 0;
                        }
                    }
                    ...
                    // 魅了された場合
                    object argIndex7 = "魅了";
                    if (withBlock7.IsConditionSatisfied(argIndex7) & ReferenceEquals(withBlock7.Master, SelectedTarget))
                    {
                        SelectedWeapon = -1;
                    }
                    ...
                    // 憑依された場合
                    object argIndex8 = "憑依";
                    if (withBlock7.IsConditionSatisfied(argIndex8))
                    {
                    ...
                    // 踊らされた場合
                    object argIndex9 = "踊り";
                    if (withBlock7.IsConditionSatisfied(argIndex9))
                    {
                        SelectedWeapon = -1;
                    }
                    ...
                    else if (SelectedWeapon == 0)
                    {
                        ...
                        if (withBlock7.IsAnimationDefined(argmain_situation2, sub_situation: argsub_situation2))
                        {
                            string argmain_situation = "射程外";
                            string argsub_situation = "";
                            withBlock7.PlayAnimation(argmain_situation, sub_situation: argsub_situation);
                        }
                        else
                        {
                            string argmain_situation1 = "射程外";
                            string argsub_situation1 = "";
                            withBlock7.SpecialEffect(argmain_situation1, sub_situation: argsub_situation1);
                        }

                        string argSituation = "射程外";
                        string argmsg_mode = "";
                        withBlock7.PilotMessage(argSituation, msg_mode: argmsg_mode);
                    }
                }
                else
                {
