...
namespace SRCCore.Units
{
    public partial class Unit
    {
        ...
        public Unit LookForAttackHelp(short tx, short ty)
        {
        ...
        public Unit LookForGuardHelp(Unit t, short tw, bool is_critical)
        {
        ...
        // 最大サポートアタック回数
        public short MaxSupportAttack()
        {
            short MaxSupportAttackRet = default;
            {
                ...
                object argIndex2 = "援護";
                string argref_mode1 = "";
                MaxSupportAttackRet = (short)GeneralLib.MaxLng((int)withBlock.SkillLevel(argIndex1, ref_mode: argref_mode), (int)withBlock.SkillLevel(argIndex2, ref_mode: argref_mode1));
            }
        ...
        // 最大サポートガード回数
        public short MaxSupportGuard()
        {
            short MaxSupportGuardRet = default;
            {
                var withBlock = MainPilot();
                ...
                object argIndex2 = "援護";
                string argref_mode1 = "";
                MaxSupportGuardRet = (short)GeneralLib.MaxLng((int)withBlock.SkillLevel(argIndex1, ref_mode: argref_mode), (int)withBlock.SkillLevel(argIndex2, ref_mode: argref_mode1));
            }
        ...
        // 最大同時援護攻撃回数
        public short MaxSyncAttack()
        {
            short MaxSyncAttackRet = default;
            object argIndex1 = "統率";
            string argref_mode = "";
            MaxSyncAttackRet = (short)MainPilot().SkillLevel(argIndex1, ref_mode: argref_mode);
            return MaxSyncAttackRet;
        }

        // 最大カウンター攻撃回数
        public short MaxCounterAttack()
        {
            short MaxCounterAttackRet = default;
            {
                var withBlock = MainPilot();
                object argIndex1 = "カウンター";
                string argref_mode = "";
                MaxCounterAttackRet = (short)withBlock.SkillLevel(argIndex1, ref_mode: argref_mode);
        ...
        // いるかどうかを判定
        public Unit LookForSupportGuard(Unit t, short tw)
        {
        ...
        public short LookForSupport(short tx, short ty, bool for_attack = false)
        {
            short LookForSupportRet = default;
            Unit u;
            short i;
            ...
            for (i = 1; i <= 4; i++)
            {
...

                {
                    ...
                    if (do_support)
                    {
                        LookForSupportRet = (short)(LookForSupportRet + 1);
                    }
        ...
        public void CombinationPartner(string ctype_Renamed, short w, Unit[] partners, short tx = 0, short ty = 0, bool check_formation = false)
        {
            Unit u;
            string uname;
            short j, i, k;
            short clevel = default, cnum = default;
            var clist = default(string);
            string cname;
            short cmorale, cen, cplana = default;
            short crange, loop_limit;
            ...
            if (ctype_Renamed == "武装")
            {
                ...
                {
                    string argattr = "霊";
                    cplana = (short)(5d * WeaponLevel(w, argattr));
                }
                else if (IsWeaponClassifiedAs(w, argattr3))
                {
                    string argattr1 = "プ";
                    cplana = (short)(5d * WeaponLevel(w, argattr1));
                }
            ...
            else
            {
                ...
                {
                    string argattr4 = "霊";
                    cplana = (short)(5d * AbilityLevel(w, argattr4));
                }
                else if (IsAbilityClassifiedAs(w, argattr7))
                {
                    string argattr5 = "プ";
                    cplana = (short)(5d * AbilityLevel(w, argattr5));
                }
            ...
            for (i = 1; i <= loopTo; i++)
            {
                ...
                if (Feature(argIndex4) == "合体技")
                {
                    ...
                    if ((GeneralLib.LIndex(arglist1, 1) ?? "") == (cname ?? ""))
                    {
                        ...
                        {
                            object argIndex2 = i;
                            clevel = (short)FeatureLevel(argIndex2);
                        }
                        else
                        {
                            clevel = 0;
        ...
        public bool IsCombinationAttackAvailable(short w, bool check_formation = false)
        {
                ...
                if (x > 1)
                {
                    if (Map.MapDataForUnit[x - 1, y] is object)
                    {
                        if (IsEnemy(Map.MapDataForUnit[x - 1, y]))
                        {
                            ...
                            CombinationPartner(argctype_Renamed2, w, partners, (short)(x - 1), y, check_formation);
                        }
                    }
                }
                    ...
                    if (x < Map.MapWidth)
                    {
                        if (Map.MapDataForUnit[x + 1, y] is object)
                        {
                            if (IsEnemy(Map.MapDataForUnit[x + 1, y]))
                            {
                                ...
                                CombinationPartner(argctype_Renamed3, w, partners, (short)(x + 1), y, check_formation);
                            }
                        }
                    }
                }
                    ...
                    if (y > 1)
                    {
                        if (Map.MapDataForUnit[x, y - 1] is object)
                        {
                            if (IsEnemy(Map.MapDataForUnit[x, y - 1]))
                            {
                                ...
                                CombinationPartner(argctype_Renamed4, w, partners, x, (short)(y - 1), check_formation);
                            }
                        }
                    }
                }
                    ...
                    if (y < Map.MapHeight)
                    {
                        if (Map.MapDataForUnit[x, y + 1] is object)
                        {
                            if (IsEnemy(Map.MapDataForUnit[x, y + 1]))
                            {
                                ...
                                CombinationPartner(argctype_Renamed5, w, partners, x, (short)(y + 1), check_formation);
                            }
                        }
                    }
                }
        ...
        public bool IsCombinationAbilityAvailable(short a, bool check_formation = false)
        {
                ...
                if (x > 1)
                {
                    if (Map.MapDataForUnit[x - 1, y] is object)
                    {
                        if (IsAlly(Map.MapDataForUnit[x - 1, y]))
                        {
                            ...
                            CombinationPartner(argctype_Renamed2, a, partners, (short)(x - 1), y, check_formation);
                        }
                    }
                }
                    ...
                    if (x < Map.MapWidth)
                    {
                        if (Map.MapDataForUnit[x + 1, y] is object)
                        {
                            if (IsAlly(Map.MapDataForUnit[x + 1, y]))
                            {
                                ...
                                CombinationPartner(argctype_Renamed3, a, partners, (short)(x + 1), y, check_formation);
                            }
                        }
                    }
                }
                    ...
                    if (y > 1)
                    {
                        if (Map.MapDataForUnit[x, y - 1] is object)
                        {
                            if (IsAlly(Map.MapDataForUnit[x, y - 1]))
                            {
                                ...
                                CombinationPartner(argctype_Renamed4, a, partners, x, (short)(y - 1), check_formation);
                            }
                        }
                    }
                }
                    ...
                    if (y > Map.MapHeight)
                    {
                        if (Map.MapDataForUnit[x, y + 1] is object)
                        {
                            if (IsAlly(Map.MapDataForUnit[x, y + 1]))
                            {
                                ...
                                CombinationPartner(argctype_Renamed5, a, partners, x, (short)(y + 1), check_formation);
                            }
                        }
                    }
                }
