...
namespace SRCCore.CmdDatas.Commands
{
    public class WaitCmd : CmdData
    {
        ...
        {
            // Waitコマンドによる「待ち」はSRC本体自身の実行を一時停止するだけでなく、
            // PC上で実行されている他のアプリケーションに対してプログラムの実行権を渡す動作を伴います。
            // これによりアニメ表示など長時間に渡るイベントコマンドの実行の際にも
            // 他のアプリケーションの実行を阻害しないようにすることが出来ます。
            // 「Wait 0」のように待ち時間を0にした場合でも実行権の譲渡は有効です。
