...
ErrorHandler: 
	...
	Public Sub PrepareWeaponAnimation(ByRef u As Unit, ByVal w As Short)
		...
		'二刀流？
		If InStr(wname, "ダブル") > 0 Or InStr(wname, "ツイン") > 0 Or InStr(wname, "双") > 0 Or InStr(wname, "二刀") > 0 Then
			double_weapon = True
		End If
		...
		If InStrNotNest(wclass, "武") = 0 And InStrNotNest(wclass, "突") = 0 And InStrNotNest(wclass, "接") = 0 And InStrNotNest(wclass, "実") = 0 Then
		...
		If InStrNotNest(wclass, "武") > 0 Then
			...
			For i = 1 To u.CountItem
				With u.Item(i)
					If .Activated And (.Part = "両手" Or .Part = "片手" Or .Part = "武器") Then
		...
		If InStrNotNest(wclass, "突") > 0 Or InStrNotNest(wclass, "接") > 0 Then
			GoTo SkipShootingWeapon
		End If
...
SkipInfightWeapon: 
		...
		'手持ち？
		If InStr(wname, "ライフル") > 0 Or InStr(wname, "バズーカ") > 0 Or Right(wname, 2) = "ガン" Or (Right(wname, 1) = "銃" And Right(wname, 2) <> "機銃") Then
			...
			ElseIf CountAttack0(u, w) >= 4 Then 
				wtype = "マシンガン"
			ElseIf InStr(wname, "ピストル") > 0 Or InStr(wname, "ミニ") > 0 Or InStr(wname, "小") > 0 Then 
				wtype = "レーザーガン"
			Else
				If double_weapon Then
					wtype = "ダブルビームライフル"
...
SkipBeamWeapon: 
		...
		If InStr(wname, "弓") > 0 Or InStr(wname, "ショートボウ") > 0 Or InStr(wname, "ロングボウ") > 0 Then
			wtype = "弓"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "ライフル") > 0 Or (Right(wname, 1) = "銃" And Right(wname, 2) <> "機銃") Or Right(wname, 2) = "ガン" Then
			wtype = "ライフル"
			GoTo FoundWeaponType
		End If
...
FoundWeaponType: 
		...
		'色
		If InStr(wtype, "ビームサーベル") > 0 Or InStr(wtype, "ビームカッター") > 0 Or wtype = "ビームナイフ" Or wtype = "ライトセイバー" Then
...
SkipWeaponAnimation: 
	...
	Private Function CheckWeaponType(ByRef wname As String, ByRef wclass As String) As String
		If InStr(wname, "ビーム") > 0 Or InStr(wname, "プラズマ") > 0 Or InStr(wname, "レーザー") > 0 Or InStr(wname, "ブラスター") > 0 Or InStr(wname, "ライト") > 0 Then
			If InStr(wname, "サーベル") > 0 Or InStr(wname, "セイバー") > 0 Or InStr(wname, "ブレード") > 0 Or InStr(wname, "ソード") > 0 Or InStr(wname, "剣") > 0 Or InStr(wname, "刀") > 0 Then
				If InStr(wname, "ハイパー") > 0 Or InStr(wname, "ロング") > 0 Or InStr(wname, "大") > 0 Or InStr(wname, "高") > 0 Then
			...
			If InStr(wname, "ナイフ") > 0 Or InStr(wname, "ダガー") > 0 Then
				CheckWeaponType = "ビームナイフ"
				Exit Function
			End If
		End If
		
		If InStr(wname, "ナイフ") > 0 Or InStr(wname, "ダガー") > 0 Or InStr(wname, "短刀") > 0 Or InStr(wname, "小刀") > 0 Then
			If InStr(wname, "投") > 0 Or InStr(wname, "飛び") > 0 Or Right(wname, 3) = "スロー" Or Right(wname, 3) = "スロウ" Or InStrNotNest(wclass, "実") > 0 Then
		...
		If InStr(wname, "ショートソード") > 0 Or InStr(wname, "短剣") > 0 Or InStr(wname, "スモールソード") > 0 Or InStr(wname, "小剣") > 0 Then
			CheckWeaponType = "ショートソード"
			Exit Function
		End If
		
		If InStr(wname, "グレートソード") > 0 Or InStr(wname, "大剣") > 0 Or InStr(wname, "ハンデッドソード") > 0 Or InStr(wname, "両手剣") > 0 Then
			CheckWeaponType = "大剣"
			Exit Function
		End If
		
		If InStr(wname, "ロングソード") > 0 Or InStr(wname, "長剣") > 0 Or InStr(wname, "バスタードソード") > 0 Or wname = "ソード" Then
			CheckWeaponType = "ソード"
			Exit Function
		End If
		...
		If InStr(wname, "シミター") > 0 Or InStr(wname, "サーベル") > 0 Or InStr(wname, "カットラス") > 0 Or InStr(wname, "三日月刀") > 0 Then
		...
		If InStr(wname, "ナギナタ") > 0 Or InStr(wname, "薙刀") > 0 Or InStr(wname, "グレイブ") > 0 Then
		...
		If InStr(wname, "アックス") > 0 Or InStr(wname, "斧") > 0 Then
			If InStr(wname, "グレート") > 0 Or InStr(wname, "両") > 0 Or InStr(wname, "バトル") > 0 Then
				CheckWeaponType = "両刃斧"
			Else
				CheckWeaponType = "片刃斧"
			End If
		...
		If Right(wname, 2) = "ムチ" Or InStr(wname, "鞭") > 0 Or InStr(wname, "ウィップ") > 0 Then
			CheckWeaponType = "鞭"
			Exit Function
		End If
		...
		If InStr(wname, "トライデント") > 0 Or InStr(wname, "三叉槍") > 0 Or InStr(wname, "ジャベリン") > 0 Then
	...
	Public Sub PrepareWeaponSound(ByRef u As Unit, ByVal w As Short)
		...
		If InStrNotNest(wclass, "武") > 0 Or InStrNotNest(wclass, "突") > 0 Then
			If InStr(wname, "ビーム") > 0 Or InStr(wname, "プラズマ") > 0 Or InStr(wname, "レーザー") > 0 Or InStr(wname, "ブラスター") > 0 Or InStr(wname, "高周波") > 0 Or InStr(wname, "電磁") > 0 Or wname = "セイバー" Or wname = "ライトセイバー" Or wname = "ランサー" Then
	...
	Public Sub AttackAnimation(ByRef u As Unit, ByVal w As Short)
		...
		'二刀流？
		If InStr(wname, "ダブル") > 0 Or InStr(wname, "ツイン") > 0 Or InStr(wname, "デュアル") > 0 Or InStr(wname, "双") > 0 Or InStr(wname, "二刀") > 0 Or InStr(wname, "２連") > 0 Or InStr(wname, "二連") > 0 Or InStr(wname, "連装") > 0 Then
			double_weapon = True
		End If
		
		'連続攻撃？
		If InStr(wname, "ダブル") > 0 Or InStr(wname, "ツイン") > 0 Or InStr(wname, "コンビネーション") > 0 Or InStr(wname, "コンボ") > 0 Or InStr(wname, "連") > 0 Or InStrNotNest(wclass, "連") > 0 Then
			double_attack = True
		End If
		
		'乱打？
		If InStr(wname, "乱打") > 0 Or InStr(wname, "乱舞") > 0 Or InStr(wname, "乱れ") > 0 Or InStr(wname, "百烈") > 0 Or (Right(wname, 4) = "ラッシュ" And InStr(wname, "クラッシュ") = 0 And InStr(wname, "スラッシュ") = 0 And InStr(wname, "スプラッシュ") = 0 And InStr(wname, "フラッシュ") = 0) Then
			combo_attack = True
		End If
		...
		'まずは白兵戦用武器の判定
		If InStrNotNest(wclass, "武") = 0 And InStrNotNest(wclass, "突") = 0 And InStrNotNest(wclass, "接") = 0 And InStrNotNest(wclass, "格") = 0 Then
			GoTo SkipInfightWeapon
		End If
		...
		If InStr(wname, "突撃") > 0 Or InStr(wname, "突進") > 0 Or InStr(wname, "チャージ") > 0 Then
		...
		If InStr(wname, "拳法") > 0 Or Right(wname, 2) = "アーツ" Or Right(wname, 5) = "ストライク" Then
			wtype = "連打"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "パンチ") > 0 Or InStr(wname, "チョップ") > 0 Or InStr(wname, "ナックル") > 0 Or InStr(wname, "ブロー") > 0 Or InStr(wname, "拳") > 0 Or InStr(wname, "掌") > 0 Or InStr(wname, "打") > 0 Or InStr(wname, "勁") > 0 Or InStr(wname, "殴") > 0 Or Right(wname, 1) = "手" Or Right(wname, 1) = "腕" Then
		...
		If InStr(wname, "タックル") > 0 Or InStr(wname, "体当") > 0 Or InStr(wname, "チャージ") > 0 Or InStr(wname, "ぶちかまし") > 0 Or InStr(wname, "かみつき") > 0 Then
			wtype = "タックル"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "キック") > 0 Or InStr(wname, "蹴") > 0 Or InStr(wname, "脚") > 0 Or Right(wname, 1) = "足" Then
			wtype = "キック"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "ソード") > 0 Or InStr(wname, "剣") > 0 Or InStr(wname, "ナイフ") > 0 Or InStr(wname, "ダガー") > 0 Or InStr(wname, "シミター") > 0 Or InStr(wname, "サーベル") > 0 Or InStr(wname, "カットラス") > 0 Or InStr(wname, "カッター") > 0 Or Right(wname, 2) = "ムチ" Or InStr(wname, "鞭") > 0 Or InStr(wname, "ウィップ") > 0 Or InStr(wname, "ハンマー") > 0 Or InStr(wname, "ロッド") > 0 Or InStr(wname, "クロー") > 0 Or InStr(wname, "爪") > 0 Or InStr(wname, "ひっかき") > 0 Or InStr(wname, "アーム") > 0 Or Right(wname, 1) = "尾" Then
			If combo_attack Then
				wtype = "白兵乱撃"
			ElseIf double_attack Then 
				wtype = "白兵連撃"
		...
		If InStr(wname, "刀") > 0 Or InStr(wname, "斬") > 0 Or InStr(wname, "ブレード") > 0 Or InStr(wname, "刃") > 0 Or InStr(wname, "アックス") > 0 Or InStr(wname, "斧") > 0 Or InStr(wname, "カット") > 0 Or InStr(wname, "カッター") > 0 Or InStr(wname, "スラッシュ") > 0 Or InStr(wname, "居合") > 0 Then
			If combo_attack Then
				wtype = "白兵乱撃"
			ElseIf double_attack Then 
				wtype = "ダブル斬撃"
		...
		If InStr(wname, "サイズ") > 0 Or InStr(wname, "鎌") > 0 Or InStr(wname, "グレイブ") > 0 Or InStr(wname, "ナギナタ") > 0 Then
			wtype = "振り下ろし"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "スピア") > 0 Or InStr(wname, "槍") > 0 Or InStr(wname, "ランス") > 0 Or InStr(wname, "ランサー") > 0 Or InStr(wname, "トライデント") > 0 Or InStr(wname, "ジャベリン") > 0 Or InStr(wname, "レイピア") > 0 Or wname = "ロッド" Then
			If combo_attack Then
				wtype = "乱突"
			ElseIf double_attack Then 
				wtype = "連突"
		...
		'詳細が分からなかった近接技
		If InStrNotNest(wclass, "突") > 0 And InStrNotNest(wclass, "接") > 0 Then
			wtype = "格闘"
			GoTo FoundWeaponType
		End If
...
SkipInfightWeapon: 
		...
		If InStr(wname, "ナイフ") > 0 Or InStr(wname, "ダガー") > 0 Or InStr(wname, "クナイ") > 0 Or InStr(wname, "苦無") > 0 Then
			wtype = "投げナイフ"
			GoTo FoundWeaponType
		End If
...
SkipThrowingWeapon: 
		...
		If InStr(wname, "ライフル") > 0 Or (Right(wname, 1) = "銃" And Right(wname, 2) <> "機銃") Then
			wtype = "ライフル"
			GoTo FoundWeaponType
		End If
...
SkipNormalHandWeapon: 
		...
		If InStr(wname, "ライフル") > 0 Or InStr(wname, "ガン") > 0 Or InStr(wname, "ピストル") > 0 Or InStr(wname, "バズーカ") > 0 Or (Right(wname, 1) = "銃" And Right(wname, 2) <> "機銃") Then
			If InStrNotNest(wclass, "Ｍ") > 0 Then
				wtype = "ＭＡＰバスタービームライフル"
				GoTo FoundWeaponType
			End If
...
SkipHandWeapon: 
		...
		If InStr(wname, "ミサイル") > 0 Or InStr(wname, "ロケット") > 0 Then
			...
			If InStr(wname, "大型") > 0 Or InStr(wname, "ビッグ") > 0 Or InStr(wname, "対艦") > 0 Then
				wtype = "スーパーミサイル"
				attack_times = 1
			ElseIf InStr(wname, "小型") > 0 Then 
				wtype = "小型ミサイル"
			ElseIf InStr(wname, "ランチャー") > 0 Or InStr(wname, "ポッド") > 0 Or InStr(wname, "マイクロ") > 0 Or InStr(wname, "スプレー") > 0 Then 
				wtype = "小型ミサイル"
				attack_times = 6
			End If
		...
		If InStr(wname, "グレネード") > 0 Or InStr(wname, "ディスチャージャー") > 0 Then
			wtype = "グレネード"
		...
		If InStr(wname, "爆弾") > 0 Or InStr(wname, "爆撃") > 0 Or InStr(wname, "爆雷") > 0 Then
		...
		If InStr(wname, "重力") > 0 Or InStr(wname, "グラビ") > 0 Or InStr(wname, "ブラックホール") > 0 Or InStr(wname, "縮退") > 0 Then
			wtype = "重力弾"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "電撃") > 0 Or InStr(wname, "電流") > 0 Or InStr(wname, "エレクト") > 0 Then
			wtype = "破壊光線"
			sname = "Thunder.wav"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "音波") > 0 Or InStr(wname, "サウンド") > 0 Or InStr(wname, "ソニック") > 0 Or (InStrNotNest(wclass, "音") > 0 And InStr(wname, "ショック") > 0) Or InStr(wname, "ウェーブ") > 0 Or InStr(wname, "叫び") > 0 Or (InStrNotNest(wclass, "音") > 0 And InStr(wname, "咆哮") > 0) Then
			wtype = "音波"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "竜巻") > 0 Or InStr(wname, "渦巻") > 0 Or InStr(wname, "トルネード") > 0 Or InStr(wname, "サイクロン") > 0 Then
			wtype = "竜巻"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "吹雪") > 0 Or InStr(wname, "ブリザード") > 0 Or InStr(wname, "アイスストーム") > 0 Then
			wtype = "吹雪"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "ストーム") > 0 Or InStr(wname, "ハリケーン") > 0 Or InStr(wname, "タイフーン") > 0 Or InStr(wname, "台風") > 0 Or InStr(wname, "嵐") > 0 Then
			wtype = "強風"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "ウィンド") > 0 Or InStr(wname, "ウインド") > 0 Or InStr(wname, "風") > 0 Then
			wtype = "風"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "煙") > 0 Or InStr(wname, "スモーク") > 0 Or Right(wname, 2) = "ガス" Or Right(wname, 1) = "霧" Or InStr(wname, "胞子") > 0 Then
			wtype = "煙"
		...
		If InStr(wname, "火炎放射") > 0 Or Right(wname, 2) = "火炎" Then
			wtype = "火炎放射"
		...
		If wtype = "ビーム" Then
			...
			ElseIf CountAttack0(u, w) >= 4 Or InStr(wname, "対空") > 0 Then 
				wtype = "ニードルレーザー連射"
			ElseIf InStr(wname, "ミニ") > 0 Or InStr(wname, "小") > 0 Then 
				wtype = "ニードルレーザー"
			...
			If InStr(wname, "拡散") > 0 Or InStr(wname, "放射") > 0 Or InStr(wname, "ホーミング") > 0 Or InStr(wname, "誘導") > 0 Then
				wtype = "拡散ビーム"
			End If
		...
		If InStr(wname, "マシンキャノン") > 0 Or InStr(wname, "オートキャノン") > 0 Or InStr(wname, "速射砲") > 0 Then
			wtype = "重機関砲"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "砲") > 0 Or InStr(wname, "キャノン") > 0 Or InStr(wname, "カノン") > 0 Or InStr(wname, "弾") > 0 Then
			If InStr(wname, "リニア") > 0 Or InStr(wname, "レール") > 0 Or InStr(wname, "電磁") > 0 Then
				PlayWave("Thunder.wav")
				Sleep(300)
			End If
...
FoundWeaponType: 
		...
		'内蔵武器を優先する。
		If is_handy_weapon And (u.Data.Transportation = "空" Or (IsOptionDefined("等身大基準") And Not u.IsHero())) Then
			Select Case wtype
				Case "ＭＡＰバスタービームライフル"
					wtype = "ＭＡＰビーム"
				Case "バスタービームライフル"
		...
		If InStrNotNest(wclass, "Ｍ") > 0 Then
			'マップ攻撃対応アニメに置き換え
			Select Case wtype
				Case "矢", "小型ミサイル", "ミサイル", "スーパーミサイル", "グレネード", "キャノン砲", "大キャノン砲", "ＩＣＢＭ", "シュートカッター", "球電", "氷弾", "火炎弾", "岩弾", "発光", "落雷", "放電", "氷柱", "つらら", "凍結", "吹雪", "風", "強風", "竜巻", "津波", "泡", "音符", "オールレンジ", "煙", "気弾", "連気弾", "気斬", "波動放射"
					wtype = "ＭＡＰ" & wtype
				Case "炎", "炎投射", "火炎放射"
					wtype = "ＭＡＰ炎"
				Case "ニードル", "ニードル連射"
				...
				Case Else
					...
					ElseIf InStr(wname, "ダーク") > 0 Or InStr(wname, "闇") > 0 Then 
						wtype = "ＭＡＰダークネス"
					ElseIf InStr(wname, "地震") > 0 Or InStr(wname, "クウェイク") > 0 Or InStr(wname, "クエイク") > 0 Then 
						wtype = "ＭＡＰ地震"
		...
		ElseIf InStr(wname, "ブラウン") > 0 Or InStr(wname, "橙") > 0 Then 
			cname = "橙"
		ElseIf InStr(wname, "ブラック") > 0 Or InStr(wname, "黒") > 0 Or InStr(wname, "ダーク") > 0 Or InStr(wname, "闇") > 0 Then 
			cname = "黒"
		ElseIf InStr(wname, "ホワイト") > 0 Or InStr(wname, "白") > 0 Or InStr(wname, "ホーリー") > 0 Or InStr(wname, "聖") > 0 Then 
			cname = "白"
		End If
	...
	Public Sub AttackSound(ByRef u As Unit, ByVal w As Short)
		...
		'効果音が必要ないもの
		If u.IsWeaponClassifiedAs(w, "武") Or u.IsWeaponClassifiedAs(w, "突") Or u.IsWeaponClassifiedAs(w, "接") Then
			Exit Sub
		End If
		...
		ElseIf InStr(wname, "対空砲") > 0 Then 
			...
			End If
		ElseIf InStr(wname, "レーザー") > 0 Or InStr(wname, "光線") > 0 Or InStr(wname, "凝集光") > 0 Or InStr(wname, "熱線") > 0 Or InStr(wname, "冷線") > 0 Or InStr(wname, "衝撃波") > 0 Or InStr(wname, "電磁波") > 0 Or InStr(wname, "電波") > 0 Or InStr(wname, "音波") > 0 Or InStr(wname, "磁力") > 0 Or InStr(wname, "ブラックホール") > 0 Or InStr(wname, "縮退") > 0 Or InStr(wname, "ウェーブ") > 0 Or InStr(wname, "波動") > 0 Or InStr(wname, "ソニック") > 0 Or InStr(wname, "スパーク") > 0 Or InStr(wname, "エネルギー") > 0 Then 
			sname = "LaserGun.wav"
		ElseIf InStr(wname, "粒子") > 0 Or InStr(wname, "陽電子") > 0 Or InStr(wname, "陽子") > 0 Or InStr(wname, "ブラスター") > 0 Or InStr(wname, "ブラスト") > 0 Or InStr(wname, "フェイザー") > 0 Or InStr(wname, "ディスラプター") > 0 Or InStr(wname, "スマッシャー") > 0 Or InStr(wname, "スラッシャー") > 0 Or InStr(wname, "フラッシャー") > 0 Or InStr(wname, "ディバイダー") > 0 Or InStr(wname, "ドライバー") > 0 Or InStr(wname, "シュトラール") > 0 Or InStr(wname, "ニュートロン") > 0 Or InStr(wname, "プラズマ") > 0 Or InStr(wname, "イオン") > 0 Or InStr(wname, "プロミネンス") > 0 Or InStr(wname, "ハイドロ") > 0 Or InStr(wname, "インパルス") > 0 Or InStr(wname, "フレイム") > 0 Or InStr(wname, "サンシャイン") > 0 Then 
			sname = "Beam.wav"
		...
		ElseIf InStr(wname, "ビーム") > 0 Then 
			...
			If InStr(wname, "バルカン") > 0 Or InStr(wname, "マシンガン") > 0 Then
				num = 4
			End If
		ElseIf InStr(wname, "機関銃") > 0 Or InStr(wname, "機銃") > 0 Or InStr(wname, "マシンガン") > 0 Or InStr(wname, "アサルトライフル") > 0 Or InStr(wname, "チェーンライフル") > 0 Or InStr(wname, "パレットライフル") > 0 Or InStr(wname, "マウラー砲") > 0 Or InStr(wname, "ＳＭＧ") > 0 Then 
			If InStrNotNest(wclass, "Ｂ") > 0 Then
				sname = "LaserGun.wav"
			Else
			...
			End If
			num = 1
		ElseIf InStr(wname, "機関砲") > 0 Or InStr(wname, "速射砲") > 0 Or InStr(wname, "マシンキャノン") > 0 Or InStr(wname, "モーターカノン") > 0 Or InStr(wname, "ガンクラスター") > 0 Or InStr(wname, "チェーンガン") > 0 Then 
			If InStrNotNest(wclass, "Ｂ") > 0 Then
				sname = "LaserGun.wav"
			Else
				sname = "MachineCannon.wav"
			End If
			num = 1
		ElseIf InStr(wname, "ガンポッド") > 0 Or InStr(wname, "バルカン") > 0 Or InStr(wname, "ガトリング") > 0 Or InStr(wname, "ハンドレールガン") > 0 Then 
			If InStrNotNest(wclass, "Ｂ") > 0 Then
				sname = "LaserGun.wav"
			Else
				sname = "GunPod.wav"
			End If
			num = 1
		ElseIf InStr(wname, "リニアキャノン") > 0 Or InStr(wname, "レールキャノン") > 0 Or InStr(wname, "リニアカノン") > 0 Or InStr(wname, "レールカノン") > 0 Or InStr(wname, "リニアガン") > 0 Or InStr(wname, "レールガン") > 0 Or (InStr(wname, "電磁") > 0 And InStr(wname, "砲") > 0) Then 
			PlayWave("Thunder.wav")
			Sleep(300)
			PlayWave("Cannon.wav")
			For i = 2 To num
		...
		ElseIf InStr(wname, "ライフル") > 0 Then 
			...
			Else
				sname = "Rifle.wav"
			End If
		ElseIf InStr(wname, "バズーカ") > 0 Or InStr(wname, "ジャイアントバズ") > 0 Or InStr(wname, "シュツルムファウスト") > 0 Or InStr(wname, "グレネード") > 0 Or InStr(wname, "グレネイド") > 0 Or InStr(wname, "ナパーム") > 0 Or InStr(wname, "クレイモア") > 0 Or InStr(wname, "ロケット砲") > 0 Or InStr(wname, "迫撃砲") > 0 Or InStr(wname, "無反動砲") > 0 Then 
			If InStrNotNest(wclass, "Ｂ") > 0 Then
				sname = "Beam.wav"
			Else
				sname = "Bazooka.wav"
			End If
		ElseIf InStr(wname, "自動砲") > 0 Or InStr(wname, "オートキャノン") > 0 Then 
			sname = "FastGun.wav"
			num = 1
		ElseIf InStr(wname, "弓") > 0 Or InStr(wname, "アロー") > 0 Or InStr(wname, "ボーガン") > 0 Or InStr(wname, "ボウガン") > 0 Or InStr(wname, "ロングボウ") > 0 Or InStr(wname, "ショートボウ") > 0 Or InStr(wname, "針") > 0 Or InStr(wname, "髪") > 0 Then 
			sname = "Bow.wav"
		ElseIf InStr(wname, "マイン") > 0 Or InStr(wname, "クラッカー") > 0 Or InStr(wname, "手投弾") > 0 Or InStr(wname, "手榴弾") > 0 Or InStr(wname, "投げ") > 0 Or InStr(wname, "スリング") > 0 Or InStr(wname, "手裏剣") > 0 Or InStr(wname, "苦無") > 0 Or InStr(wname, "クナイ") > 0 Then 
			sname = "Swing.wav"
		ElseIf InStr(wname, "爆弾") > 0 Or InStr(wname, "爆雷") > 0 Or InStr(wname, "爆撃") > 0 Then 
			sname = "Bomb.wav"
		ElseIf InStr(wname, "機雷") > 0 Then 
			sname = "Explode.wav"
		ElseIf InStr(wname, "マイクロミサイル") > 0 And InStrNotNest(wclass, "Ｍ") > 0 Then 
			sname = "MicroMissile.wav"
			num = 1
		ElseIf InStr(wname, "全方位ミサイル") > 0 Then 
			sname = "MicroMissile.wav"
			num = 1
		ElseIf InStr(wname, "ミサイル") > 0 Or InStr(wname, "ロケット") > 0 Or InStr(wname, "魚雷") > 0 Or InStr(wname, "反応弾") > 0 Or InStr(wname, "マルチポッド") > 0 Or InStr(wname, "マルチランチャー") > 0 Or InStr(wname, "ショット") > 0 Or InStr(wname, "フルファイア") > 0 Or InStr(wname, "ストリーム") > 0 Or InStr(wname, "ナックル") > 0 Or InStr(wname, "パンチ") > 0 Or InStr(wname, "鉄腕") > 0 Or InStr(wname, "発射") > 0 Or InStr(wname, "射出") > 0 Or InStr(wname, "ランチャー") > 0 Or InStr(wname, "ＡＴＭ") > 0 Or InStr(wname, "ＡＡＭ") > 0 Or InStr(wname, "ＡＧＭ") > 0 Then 
			If InStrNotNest(wclass, "Ｂ") > 0 Then
				sname = "Beam.wav"
			Else
				sname = "Missile.wav"
			End If
		ElseIf InStr(wname, "砲") > 0 Or InStr(wname, "弾") > 0 Or InStr(wname, "キャノン") > 0 Or InStr(wname, "カノン") > 0 Or InStr(wname, "ボム") > 0 Or InStr(wname, "火球") > 0 Then 
			If InStrNotNest(wclass, "Ｂ") > 0 Then
				sname = "Beam.wav"
			Else
				sname = "Cannon.wav"
			End If
		ElseIf InStr(wname, "ガン") > 0 Or InStr(wname, "ピストル") > 0 Or InStr(wname, "リボルヴァー") > 0 Or InStr(wname, "マグナム") > 0 Or InStr(wname, "ライアット") > 0 Or InStr(wname, "銃") > 0 Then 
			If InStrNotNest(wclass, "Ｂ") > 0 Then
				sname = "Beam.wav"
			Else
				sname = "Gun.wav"
			End If
		ElseIf InStr(wname, "ソニックブレード") > 0 Or InStr(wname, "ビームカッター") > 0 Or InStr(wname, "スライサー") > 0 Then 
			sname = "Saber.wav"
		ElseIf InStr(wname, "重力") > 0 Or InStr(wname, "グラビ") > 0 Then 
			sname = "Shock(Low).wav"
		ElseIf InStr(wname, "ストーム") > 0 Or InStr(wname, "トルネード") > 0 Or InStr(wname, "ハリケーン") > 0 Or InStr(wname, "タイフーン") > 0 Or InStr(wname, "サイクロン") > 0 Or InStr(wname, "ブリザード") > 0 Or InStr(wname, "竜巻") > 0 Or InStr(wname, "渦巻") > 0 Or InStr(wname, "台風") > 0 Or InStr(wname, "嵐") > 0 Or InStr(wname, "吹雪") > 0 Or InStr(wname, "フリーザー") > 0 Or InStr(wname, "テレキネシス") > 0 Then 
			sname = "Storm.wav"
			num = 1
		ElseIf InStr(wname, "ブーメラン") > 0 Or InStr(wname, "ウェッブ") > 0 Then 
			sname = "Swing.wav"
			num = 5
		ElseIf InStr(wname, "サンダー") > 0 Or InStr(wname, "ライトニング") > 0 Or InStr(wname, "ボルト") > 0 Or InStr(wname, "稲妻") > 0 Or InStr(wname, "放電") > 0 Or InStr(wname, "電撃") > 0 Or InStr(wname, "電流") > 0 Or InStr(wname, "雷") > 0 Or InStrNotNest(wclass, "雷") > 0 Then 
			sname = "Thunder.wav"
			num = 1
		ElseIf InStr(wname, "火炎放射") > 0 Then 
			sname = "AntiShipMissile.wav"
		ElseIf InStr(wname, "火炎") > 0 Or InStr(wname, "焔") > 0 Then 
			sname = "Fire.wav"
			num = 1
		ElseIf InStr(wname, "魔法") > 0 Or InStrNotNest(wclass, "魔") > 0 Or InStr(wname, "サイコキネシス") > 0 Or InStr(wname, "糸") > 0 Or InStr(wname, "アンカー") > 0 Then 
			sname = "Whiz.wav"
		ElseIf InStr(wname, "泡") > 0 Or InStr(wname, "バブル") > 0 Then 
			sname = "Bubble.wav"
		ElseIf Right(wname, 1) = "液" Then 
			sname = "Shower.wav"
		ElseIf Right(wname, 3) = "ブレス" Or Right(wname, 3) = "の息" Then 
	...
	Public Sub HitAnimation(ByRef u As Unit, ByVal w As Short, ByRef t As Unit, ByVal hit_count As Short)
		...
		If InStrNotNest(wclass, "Ｍ") > 0 Then
			...
			If IsBeamWeapon(wname, wclass, cname) Or InStr(wname, "ミサイル") > 0 Or InStr(wname, "ロケット") > 0 Then
				sname = "Explode.wav"
			End If
		...
		'連続攻撃？
		If InStr(wname, "ダブル") > 0 Or InStr(wname, "ツイン") > 0 Or InStr(wname, "コンビネーション") > 0 Or InStr(wname, "連") > 0 Or InStrNotNest(wclass, "連") > 0 Then
			double_attack = True
		End If
		
		'乱打？
		If InStr(wname, "乱打") > 0 Or InStr(wname, "乱舞") > 0 Or InStr(wname, "乱れ") > 0 Or InStr(wname, "百烈") > 0 Then
			combo_attack = True
		End If
		...
		'まずは白兵戦用武器の判定
		If InStrNotNest(wclass, "武") = 0 And InStrNotNest(wclass, "突") = 0 And InStrNotNest(wclass, "接") = 0 And Not (InStrNotNest(wclass, "格") > 0 And InStrNotNest(wclass, "実") > 0) Then
			GoTo SkipInfightWeapon
		End If
		...
		If InStrNotNest(wclass, "実") > 0 And (InStr(wname, "パンチ") > 0 Or InStr(wname, "ナックル") > 0) Then
			wtype = "ロケットパンチ"
			GoTo FoundWeaponType
		End If
		...
		'通常打撃
		If InStr(wname, "パンチ") > 0 Or InStr(wname, "ナックル") > 0 Or InStr(wname, "ブロー") > 0 Or InStr(wname, "チョップ") > 0 Or InStr(wname, "ビンタ") > 0 Or InStr(wname, "殴") > 0 Or Right(wname, 1) = "手" Or Right(wname, 1) = "腕" Or InStr(wname, "格闘") > 0 Or InStr(wname, "トンファー") > 0 Or InStr(wname, "棒") > 0 Or InStr(wname, "杖") > 0 Or InStr(wname, "スタッフ") > 0 Or InStr(wname, "メイス") > 0 Or Right(wname, 2) = "ムチ" Or InStr(wname, "鞭") > 0 Or InStr(wname, "ウィップ") > 0 Or InStr(wname, "チェーン") > 0 Or InStr(wname, "ロッド") > 0 Or InStr(wname, "モーニングスター") > 0 Or InStr(wname, "フレイル") > 0 Or InStr(wname, "ヌンチャク") > 0 Or InStr(wname, "三節根") > 0 Or (InStr(wname, "チェーン") > 0 And InStr(wname, "チェーンソー") = 0) Or InStr(wname, "バット") > 0 Or InStr(wname, "ギター") > 0 Or InStr(wname, "竹刀") > 0 Or InStr(wname, "ハリセン") > 0 Then
			If combo_attack Then
				wtype = "乱打"
			ElseIf double_attack Or InStr(wname, "触手") > 0 Or InStr(wname, "触腕") > 0 Then 
				wtype = "連打"
			Else
				wtype = "打撃"
			End If
			
			If Right(wname, 2) = "ムチ" Or InStr(wname, "鞭") > 0 Or InStr(wname, "ウィップ") > 0 Or InStr(wname, "チェーン") > 0 Or InStr(wname, "触手") > 0 Or InStr(wname, "触腕") > 0 Or (InStr(wname, "ロッド") > 0 And wname <> "ロッド") Or InStr(wname, "竹刀") > 0 Or InStr(wname, "ハリセン") > 0 Then
				sname = "Whip.wav"
			ElseIf InStr(wname, "張り手") > 0 Or InStr(wname, "平手") > 0 Or InStr(wname, "ビンタ") > 0 Then 
				sname = "Slap.wav"
			End If
		...
		'強打撃
		If InStr(wname, "拳") > 0 Or InStr(wname, "掌") > 0 Or InStr(wname, "打") > 0 Or InStr(wname, "勁") > 0 Or InStr(wname, "ラリアート") > 0 Or InStr(wname, "キック") > 0 Or InStr(wname, "蹴") > 0 Or InStr(wname, "脚") > 0 Or Right(wname, 1) = "足" Or InStr(wname, "ヘッドバッド") > 0 Or InStr(wname, "頭突") > 0 Or InStr(wname, "ハンマー") > 0 Or InStr(wname, "槌") > 0 Or InStr(wname, "モール") > 0 Then
			If combo_attack Then
				wtype = "乱打"
			ElseIf double_attack Then 
				wtype = "連打"
			...
			If InStr(wname, "拳") > 0 Or InStr(wname, "掌") > 0 Or InStr(wname, "打") > 0 Or InStr(wname, "勁") > 0 Then
		...
		If InStr(wname, "タックル") > 0 Or InStr(wname, "体当") > 0 Or InStr(wname, "チャージ") > 0 Or InStr(wname, "ぶちかまし") > 0 Or InStr(wname, "バンカー") > 0 Then
			wtype = "強打"
			sname = "Crash.wav"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "ソード") > 0 Or InStr(wname, "剣") > 0 Or InStr(wname, "ナイフ") > 0 Or InStr(wname, "ダガー") > 0 Or InStr(wname, "シミター") > 0 Or InStr(wname, "サーベル") > 0 Or InStr(wname, "カットラス") > 0 Or InStr(wname, "刀") > 0 Or InStr(wname, "斬") > 0 Or InStr(wname, "ブレード") > 0 Or InStr(wname, "刃") > 0 Or InStr(wname, "アックス") > 0 Or InStr(wname, "斧") > 0 Or InStr(wname, "グレイブ") > 0 Or InStr(wname, "ナギナタ") > 0 Or InStr(wname, "切") > 0 Or InStr(wname, "裂") > 0 Or InStr(wname, "カット") > 0 Or InStr(wname, "カッター") > 0 Or InStr(wname, "スラッシュ") > 0 Or InStr(wname, "居合") > 0 Then
			If combo_attack Then
				wtype = "斬撃乱舞"
			...
			ElseIf InStrNotNest(wclass, "Ｊ") > 0 Then 
				wtype = "斬り上げ"
			ElseIf InStr(wname, "黒") > 0 Or InStr(wname, "闇") > 0 Or InStr(wname, "死") > 0 Or InStr(wname, "ダーク") > 0 Or InStr(wname, "デス") > 0 Then 
				wtype = "黒斬撃"
			Else
				wtype = "斬撃"
			End If
		...
		If InStr(wname, "噛") > 0 Or InStr(wname, "牙") > 0 Or InStr(wname, "かみつき") > 0 Then
			wtype = "噛み付き"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "スープレックス") > 0 Or InStr(wname, "投げ") > 0 Or wname = "返し" Then
			wtype = "投げ飛ばし"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "固め") > 0 Or InStr(wname, "ホールド") > 0 Or InStr(wname, "ツイスト") > 0 Or InStr(wname, "絞め") > 0 Or InStr(wname, "締め") > 0 Or InStr(wname, "折り") > 0 Then
			wtype = "立ち固め"
			GoTo FoundWeaponType
		End If
...
SkipInfightWeapon: 
		...
		If InStr(wname, "斧") > 0 Or InStr(wname, "アックス") > 0 Or InStr(wname, "トマホーク") > 0 Or InStr(wname, "ソーサー") > 0 Or InStr(wname, "チャクラム") > 0 Then
			wtype = "ダメージ"
			sname = "Saber.wav"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "パンチ") > 0 Or InStr(wname, "ハンマー") > 0 Or InStr(wname, "岩") > 0 Or InStr(wname, "鉄球") > 0 Then
			wtype = "強打"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "石") > 0 Or InStr(wname, "礫") > 0 Or InStr(wname, "分銅") > 0 Or InStr(wname, "ブーメラン") > 0 Then
			wtype = "打撃"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "ナイフ") > 0 Or InStr(wname, "ダガー") > 0 Or InStr(wname, "手裏剣") > 0 Or InStr(wname, "クナイ") > 0 Or InStr(wname, "苦無") > 0 Then
			wtype = "刺突"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "弓") > 0 Or InStr(wname, "ショートボウ") > 0 Or InStr(wname, "ロングボウ") > 0 Or InStr(wname, "ボウガン") > 0 Or InStr(wname, "矢") > 0 Or InStr(wname, "アロー") > 0 Then
			wtype = "矢"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "ショットガン") > 0 Or InStr(wname, "散弾") > 0 Or InStr(wname, "拡散バズーカ") > 0 Then
			wtype = "ショットガン"
			GoTo FoundWeaponType
		End If
...
SkipNormalWeapon: 
		...
		If wtype = "ビーム" Then
			If InStr(CurrentWeaponType, "ビーム") > 0 Or InStr(CurrentWeaponType, "レーザー") > 0 Then
			...
			Else
				...
				If InStr(wname, "拡散") > 0 Or InStr(wname, "放射") > 0 Then
					wtype = "拡散ビーム"
				End If
				
				If InStr(wname, "ホーミング") > 0 Or InStr(wname, "誘導") > 0 Then
					wtype = "ホーミングレーザー"
				End If
			End If
			GoTo FoundWeaponType
		...
		If InStr(wname, "ピストル") > 0 Or InStr(wname, "拳銃") > 0 Or InStr(wname, "リボルバー") > 0 Or InStr(wname, "リボルヴァー") > 0 Or InStr(wname, "銃") > 0 Or Right(wname, 2) = "ガン" Or InStr(wname, "ライフル") > 0 Then
			wtype = "銃弾"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "ミサイル") > 0 Or InStr(wname, "ロケット") > 0 Or InStr(wname, "爆弾") > 0 Or InStr(wname, "ダイナマイト") > 0 Or InStr(wname, "榴弾") > 0 Or InStr(wname, "反応弾") > 0 Or InStr(wname, "グレネード") > 0 Or InStr(wname, "手榴弾") > 0 Or InStr(wname, "クラッカー") > 0 Or InStr(wname, "ディスチャージャー") > 0 Or InStr(wname, "マイン") > 0 Or InStr(wname, "ボム") > 0 Or InStr(wname, "魚雷") > 0 Or InStr(wname, "機雷") > 0 Or InStr(wname, "バズーカ") > 0 Or InStr(wname, "シュツルムファウスト") > 0 Then
			If InStr(wname, "核") > 0 Or InStr(wname, "反応") > 0 Or InStr(wname, "アトミック") > 0 Or InStr(wname, "超") > 0 Then
				wtype = "超爆発"
			ElseIf InStr(wname, "大") > 0 Or InStr(wname, "ビック") > 0 Or InStr(wname, "ジャイアント") > 0 Or InStr(wname, "メガ") > 0 Then 
				wtype = "大爆発"
			ElseIf InStr(wname, "小") > 0 Or InStr(wname, "ミニ") > 0 Or InStr(wname, "マイクロ") > 0 Then 
				wtype = "小爆発"
			Else
				wtype = "爆発"
			End If
		...
		If InStr(wname, "トルネード") > 0 Or InStr(wname, "サイクロン") Or InStr(wname, "竜巻") > 0 Or InStr(wname, "渦巻") > 0 Then
			wtype = "竜巻"
			GoTo FoundWeaponType
		End If
		...
		If InStr(wname, "火") > 0 Or InStr(wname, "炎") > 0 Or InStr(wname, "焔") > 0 Or InStr(wname, "ファイヤー") > 0 Then
			wtype = "炎"
			GoTo FoundWeaponType
		End If
		
		If InStr(wname, "水鉄砲") > 0 Or InStr(wname, "放水") > 0 Or InStr(wname, "水流") > 0 Or InStr(wname, "酸かけ") > 0 Or Right(wname, 1) = "液" Or Right(wname, 1) = "酸" Then
			wtype = "飛沫"
			If InStr(wname, "毒") > 0 Or InStr(wname, "毒") > 0 Then
				cname = "緑"
		...
		If InStr(wname, "吸収") > 0 Or InStr(wname, "ドレイン") > 0 Or InStrNotNest(wclass, "吸") > 0 Or InStrNotNest(wclass, "減") > 0 Then
			wtype = "吸収"
			GoTo FoundWeaponType
		End If
...
FoundWeaponType: 
	...
	Public Sub HitSound(ByRef u As Unit, ByRef w As Short, ByRef t As Unit, ByVal hit_count As Short)
		...
		'武器に応じて効果音を再生
		If InStrNotNest(wclass, "武") > 0 Or InStrNotNest(wclass, "突") > 0 Or InStrNotNest(wclass, "接") > 0 Or InStrNotNest(wclass, "実") > 0 Then
			If InStr(wname, "ディスカッター") > 0 Or InStr(wname, "リッパー") > 0 Or InStr(wname, "スパイド") > 0 Or InStr(wname, "居合") > 0 Or InStr(wname, "閃") > 0 Then
				PlayWave("Swing.wav")
				Sleep(200)
				PlayWave("Sword.wav")
				For i = 2 To num
					...
					PlayWave("Sword.wav")
				Next 
			ElseIf InStr(wname, "プログレッシブナイフ") > 0 Or InStr(wname, "ドリル") > 0 Then 
				PlayWave("Drill.wav")
			ElseIf InStr(wname, "サーベル") > 0 Or InStr(wname, "セイバー") > 0 Or InStr(wname, "ソード") > 0 Or InStr(wname, "ブレード") > 0 Or InStr(wname, "スパッド") > 0 Or InStr(wname, "セーバー") > 0 Or InStr(wname, "ダガー") > 0 Or InStr(wname, "ナイフ") > 0 Or InStr(wname, "トマホーク") > 0 Or InStr(wname, "メイス") > 0 Or InStr(wname, "アックス") > 0 Or InStr(wname, "グレイブ") > 0 Or InStr(wname, "ナギナタ") > 0 Or InStr(wname, "ビアンキ") > 0 Or InStr(wname, "ウェッブ") > 0 Or InStr(wname, "ザンバー") > 0 Or InStr(wname, "マーカー") > 0 Or InStr(wname, "バスター") > 0 Or InStr(wname, "ブラスター") > 0 Or InStr(wname, "クロー") > 0 Or InStr(wname, "ジザース") > 0 Or InStr(wname, "ブーメラン") > 0 Or InStr(wname, "ソーサー") > 0 Or InStr(wname, "レザー") > 0 Or InStr(wname, "レイバー") > 0 Or InStr(wname, "サイズ") > 0 Or InStr(wname, "ショーテル") > 0 Or InStr(wname, "カッター") > 0 Or InStr(wname, "スパイク") > 0 Or InStr(wname, "カトラス") > 0 Or InStr(wname, "エッジ") > 0 Or (InStr(wname, "剣") > 0 And InStr(wname, "手裏剣") = 0) Or InStr(wname, "切") > 0 Or InStr(wname, "斬") > 0 Or InStr(wname, "刀") > 0 Or InStr(wname, "刃") > 0 Or InStr(wname, "斧") > 0 Or InStr(wname, "鎌") > 0 Or InStr(wname, "かま") > 0 Or InStr(wname, "カマ") > 0 Or InStr(wname, "爪") > 0 Or InStr(wname, "かぎづめ") > 0 Or InStr(wname, "ハサミ") > 0 Or InStr(wname, "バサミ") > 0 Or InStr(wname, "羽") > 0 Then 
				If Not t.IsHero Or InStr(wname, "ビーム") > 0 Or InStr(wname, "プラズマ") > 0 Or InStr(wname, "レーザー") > 0 Or InStr(wname, "セイバー") > 0 Then
					PlayWave("Saber.wav")
					For i = 2 To num
						Sleep(350)
						PlayWave("Saber.wav")
				...
				Else
					...
					Next 
				End If
			ElseIf InStr(wname, "ランサー") > 0 Or InStr(wname, "ランス") > 0 Or InStr(wname, "スピア") > 0 Or InStr(wname, "トライデント") > 0 Or InStr(wname, "ハーケン") > 0 Or InStr(wname, "槍") > 0 Or InStr(wname, "もり") > 0 Or InStr(wname, "手裏剣") > 0 Or InStr(wname, "苦無") > 0 Or InStr(wname, "クナイ") > 0 Or (InStr(wname, "突き") > 0 And InStr(wname, "拳") = 0 And InStr(wname, "頭") = 0) Then 
				If Not t.IsHero Or InStr(wname, "ビーム") > 0 Or InStr(wname, "プラズマ") > 0 Or InStr(wname, "レーザー") > 0 Or InStr(wname, "ランサー") > 0 Then
					PlayWave("Saber.wav")
					For i = 2 To num
						Sleep(350)
						PlayWave("Saber.wav")
				...
				Else
					...
					Next 
				End If
			ElseIf InStr(wname, "牙") > 0 Or InStr(wname, "ファング") > 0 Or InStr(wname, "噛") > 0 Or InStr(wname, "かみつき") > 0 Or InStr(wname, "顎") > 0 Then 
				...
				Else
					...
					Next 
				End If
			ElseIf InStr(wname, "ストライク") > 0 Or InStr(wname, "アーツ") > 0 Or InStr(wname, "拳法") > 0 Or InStr(wname, "振動拳") > 0 Then 
				PlayWave("Combo.wav")
			ElseIf InStr(wname, "格闘") > 0 Or InStr(wname, "パンチ") > 0 Or InStr(wname, "キック") > 0 Or InStr(wname, "チョップ") > 0 Or InStr(wname, "ナックル") > 0 Or InStr(wname, "ブロー") > 0 Or InStr(wname, "ハンマー") > 0 Or InStr(wname, "トンファー") > 0 Or InStr(wname, "ヌンチャク") > 0 Or InStr(wname, "パイプ") > 0 Or InStr(wname, "ラリアット") > 0 Or InStr(wname, "アーム") > 0 Or InStr(wname, "ヘッドバット") > 0 Or InStr(wname, "スリング") > 0 Or InStr(wname, "頭突き") > 0 Or InStr(wname, "脚") > 0 Or InStr(wname, "蹴") > 0 Or InStr(wname, "棒") > 0 Or InStr(wname, "石") > 0 Or InStr(wname, "角") > 0 Or InStr(wname, "尻尾") > 0 Or InStr(wname, "鉄腕") > 0 Then 
				PlayWave("Punch.wav")
				For i = 2 To num
					Sleep(120)
					PlayWave("Punch.wav")
				Next 
			ElseIf InStr(wname, "体当たり") > 0 Or InStr(wname, "タックル") > 0 Or InStr(wname, "ぶちかまし") > 0 Or InStr(wname, "突進") > 0 Or InStr(wname, "突撃") > 0 Or InStr(wname, "怪力") > 0 Or InStr(wname, "鉄拳") > 0 Or InStr(wname, "メガトンパンチ") > 0 Or InStr(wname, "鉄球") > 0 Or InStr(wname, "ボール") > 0 Or InStr(wname, "車輪") > 0 Or InStr(wname, "キャタピラ") > 0 Or InStr(wname, "シールド") > 0 Then 
				PlayWave("Crash.wav")
			ElseIf InStr(wname, "拳") > 0 Or InStr(wname, "掌") > 0 Or InStr(wname, "打") > 0 Or InStr(wname, "勁") > 0 Then 
				PlayWave("Bazooka.wav")
				For i = 2 To num
					Sleep(120)
					PlayWave("Bazooka.wav")
				Next 
			ElseIf InStr(wname, "踏み") > 0 Or InStr(wname, "押し") > 0 Or InStr(wname, "ドロップ") > 0 Then 
				PlayWave("Shock(Low).wav")
			ElseIf InStr(wname, "張り手") > 0 Or InStr(wname, "ビンタ") > 0 Then 
				PlayWave("Slap.wav")
				For i = 2 To num
					Sleep(120)
					PlayWave("Slap.wav")
				Next 
			ElseIf InStr(wname, "弓") > 0 Or InStr(wname, "矢") > 0 Or InStr(wname, "アロー") > 0 Or InStr(wname, "ボーガン") > 0 Or InStr(wname, "ボウガン") > 0 Or InStr(wname, "ショートボウ") > 0 Or InStr(wname, "ロングボウ") > 0 Or InStr(wname, "針") > 0 Or InStr(wname, "ニードル") > 0 Then 
				PlayWave("Stab.wav")
				For i = 2 To num
					Sleep(120)
					PlayWave("Stab.wav")
				Next 
			ElseIf InStr(wname, "鞭") > 0 Or InStr(wname, "ムチ") > 0 Or InStr(wname, "ウイップ") > 0 Or InStr(wname, "チェーン") > 0 Or InStr(wname, "ロッド") > 0 Or InStr(wname, "テンタク") > 0 Or InStr(wname, "テイル") > 0 Or InStr(wname, "尾") > 0 Or InStr(wname, "触手") > 0 Or InStr(wname, "触腕") > 0 Or InStr(wname, "舌") > 0 Or InStr(wname, "巻き") > 0 Or InStr(wname, "糸") > 0 Then 
				PlayWave("Whip.wav")
			ElseIf InStr(wname, "投げ") > 0 Or InStr(wname, "スープレック") > 0 Or (InStr(wname, "返し") > 0 And InStrNotNest(wclass, "突") > 0) Then 
				PlayWave("Swing.wav")
				Sleep(500)
				PlayWave("Shock(Low).wav")
				For i = 2 To num
			...
			ElseIf InStr(wname, "大雪山おろし") > 0 Then 
				...
				Sleep(300)
				PlayWave("Shock(Low).wav")
			ElseIf InStr(wname, "関節") > 0 Or InStr(wname, "固め") > 0 Or InStr(wname, "折り") > 0 Or InStr(wname, "締め") > 0 Or InStr(wname, "絞め") > 0 Or InStr(wname, "アームロック") > 0 Or InStr(wname, "ホールド") > 0 Then 
				PlayWave("Swing.wav")
				Sleep(190)
				PlayWave("BreakOff.wav")
			ElseIf InStrNotNest(wclass, "核") > 0 Or InStr(wname, "核") > 0 Or InStr(wname, "反応弾") > 0 Then 
				PlayWave("Explode(Nuclear).wav")
			ElseIf InStr(wname, "ミサイル") > 0 Or InStr(wname, "ロケット") > 0 Or InStr(wname, "魚雷") > 0 Or InStr(wname, "マルチポッド") > 0 Or InStr(wname, "マルチランチャー") > 0 Or InStr(wname, "爆弾") > 0 Or InStr(wname, "爆雷") > 0 Or InStr(wname, "爆撃") > 0 Or Right(wname, 3) = "マイン" Or Right(wname, 2) = "ボム" Then 
			...
			End If
		Else
			If InStr(wname, "ストーム") > 0 Or InStr(wname, "トルネード") > 0 Or InStr(wname, "ハリケーン") > 0 Or InStr(wname, "タイフーン") > 0 Or InStr(wname, "サイクロン") > 0 Or InStr(wname, "ブリザード") > 0 Or InStr(wname, "竜巻") > 0 Or InStr(wname, "渦巻") > 0 Or InStr(wname, "台風") > 0 Or InStr(wname, "嵐") > 0 Then
				'命中時は無音
			ElseIf Right(wname, 1) = "液" Then 
				PlayWave("Inori.wav")
			ElseIf InStr(wname, "発火") > 0 Or InStr(wname, "パイロキネシス") > 0 Then 
				PlayWave("Fire.wav")
	...
	Public Sub DodgeEffect(ByRef u As Unit, ByRef w As Short)
		...
		ElseIf InStrNotNest(wclass, "実") Then 
			If InStr(wname, "鞭") > 0 Or InStr(wname, "ムチ") > 0 Or InStr(wname, "ウイップ") > 0 Or InStr(wname, "チェーン") > 0 Or InStr(wname, "ロッド") > 0 Or InStr(wname, "テンタク") > 0 Or InStr(wname, "テイル") > 0 Or InStr(wname, "尾") > 0 Or InStr(wname, "触手") > 0 Or InStr(wname, "触腕") > 0 Or InStr(wname, "舌") > 0 Or InStr(wname, "巻き") > 0 Or InStr(wname, "糸") > 0 Then
				PlayWave("Swing.wav")
			End If
		End If
	End Sub
	...
	Public Sub ParryEffect(ByRef u As Unit, ByRef w As Short, ByRef t As Unit)
		...
		'効果音生成回数を設定
		num = CountAttack(u, w)
		If InStr(wname, "マシンガン") > 0 Or InStr(wname, "機関銃") > 0 Or InStr(wname, "アサルトライフル") > 0 Or InStr(wname, "バルカン") > 0 Then
			num = 4
		End If
		
		'命中音を設定
		If InStrNotNest(wclass, "銃") Or InStrNotNest(wclass, "格") Or InStrNotNest(wclass, "武") Or InStrNotNest(wclass, "突") Or InStr(wname, "弓") > 0 Or InStr(wname, "アロー") > 0 Or InStr(wname, "ロングボウ") > 0 Or InStr(wname, "ショートボウ") > 0 Or InStr(wname, "ボーガン") > 0 Or InStr(wname, "ボウガン") > 0 Or InStr(wname, "針") > 0 Or InStr(wname, "ニードル") > 0 Or InStr(wname, "ランサー") > 0 Or InStr(wname, "ダガー") > 0 Or InStr(wname, "剣") > 0 Then
			sname = "Sword.wav"
		ElseIf InStrNotNest(wclass, "実") Then 
	...
	Public Sub AbsorbEffect(ByRef u As Unit, ByRef w As Short, ByRef t As Unit)
		...
		'戦闘アニメオフの場合は効果音再生のみ
		If Not BattleAnimation Or IsOptionDefined("戦闘アニメ非自動選択") Then
			PlayWave("Charge.wav")
			Exit Sub
		End If
	...
	Public Sub CriticalEffect(ByRef ctype_Renamed As String, ByVal w As Short, ByVal ignore_death As Boolean)
		...
		Else
			For i = 1 To LLength(ctype_Renamed)
				aname = LIndex(ctype_Renamed, i) & "クリティカル"
...
NextLoop: 
	...
	Private Function CountAttack0(ByRef u As Unit, ByVal w As Short) As Short
		...
		If InStr(wname, "連") > 0 Then
			...
			If InStr(wname, "連打") > 0 Or InStr(wname, "連射") > 0 Or InStr(wname, "多連") > 0 Then
				CountAttack0 = 3
				Exit Function
			End If
		...
		If InStr(wname, "全弾") > 0 Or InStr(wname, "斉") > 0 Or InStr(wname, "乱射") > 0 Or InStr(wname, "フルファイア") > 0 Or InStr(wname, "スプリット") > 0 Or InStr(wname, "マルチ") > 0 Or InStr(wname, "パラレル") > 0 Or InStr(wname, "分身") > 0 Or InStr(wname, "乱打") > 0 Or InStr(wname, "乱舞") > 0 Or InStr(wname, "乱れ") > 0 Or InStr(wname, "百烈") > 0 Or InStr(wname, "千本") > 0 Or InStr(wname, "千手") > 0 Or InStr(wname, "ファンネル") > 0 Or InStr(wname, "ビット") > 0 Then
			CountAttack0 = 4
			Exit Function
		End If
		
		If InStr(wname, "マシンガン") > 0 Or InStr(wname, "機銃") > 0 Or InStr(wname, "機関銃") > 0 Or InStr(wname, "バルカン") > 0 Or InStr(wname, "ガトリング") > 0 Or (InStr(wname, "パルス") > 0 And InStr(wname, "インパルス") = 0) Or InStr(wname, "速射") > 0 Or InStr(wname, "ロケットランチャー") > 0 Or InStr(wname, "ミサイルランチャー") > 0 Or InStr(wname, "ミサイルポッド") > 0 Then
			CountAttack0 = 4
			Exit Function
		End If
		
		If InStr(wname, "トリプル") > 0 Or InStr(wname, "インコム") > 0 Or InStr(wname, "ファミリア") > 0 Or InStr(wname, "爆撃") > 0 Or InStr(wname, "爆弾") > 0 Or InStr(wname, "爆雷") > 0 Or InStr(wname, "艦載機") > 0 Then
			CountAttack0 = 3
			Exit Function
		End If
		
		If InStr(wname, "ツイン") > 0 Or InStr(wname, "ダブル") > 0 Or InStr(wname, "デュアル") > 0 Or InStr(wname, "マイクロ") > 0 Or InStr(wname, "双") > 0 Or InStr(wname, "二丁") > 0 Or InStr(wname, "二刀") > 0 Then
			CountAttack0 = 2
			Exit Function
		End If
	...
	Private Function IsBeamWeapon(ByRef wname As String, ByVal wclass As String, ByRef cname As String) As Boolean
		...
		If InStr(wname, "ビーム") > 0 Or InStrNotNest(wclass, "Ｂ") > 0 Then
			IsBeamWeapon = True
		Else
		...
		If InStr(wname, "反物質") > 0 Or InStr(wname, "熱線") > 0 Or InStr(wname, "ブラスター") > 0 Then
			IsBeamWeapon = True
			cname = "レッド"
		ElseIf InStr(wname, "フェイザー") > 0 Or InStr(wname, "粒子") > 0 Then 
			IsBeamWeapon = True
			...
			Else
				cname = "ピンク"
			End If
		ElseIf InStr(wname, "冷凍") > 0 Or InStr(wname, "冷線") > 0 Or InStr(wname, "フリーザー") > 0 Then 
			IsBeamWeapon = True
			cname = "ブルー"
		ElseIf InStr(wname, "中間子") > 0 Or InStr(wname, "中性子") > 0 Or InStr(wname, "ニュートロン") > 0 Or InStr(wname, "ニュートリノ") > 0 Then 
			IsBeamWeapon = True
			cname = "グリーン"
		ElseIf InStr(wname, "プラズマ") > 0 Then 
			IsBeamWeapon = True
			cname = "オレンジ"
		ElseIf InStr(wname, "レーザー") > 0 Or InStr(wname, "光子") > 0 Then 
			IsBeamWeapon = True
			cname = "イエロー"
		...
		If cname = "" Then
			If InStr(wname, "粒子") > 0 Then
				...
				Else
					cname = "ピンク"
				End If
			ElseIf InStr(wname, "イオン") > 0 Or InStr(wname, "冷凍") > 0 Or InStr(wname, "電子") > 0 Then 
				cname = "ブルー"
			End If
		End If
		
		If Not IsBeamWeapon And cname <> "" Then
			If Right(wname, 2) = "光線" Or Right(wname, 1) = "砲" Or Right(wname, 1) = "銃" Then
				IsBeamWeapon = True
			End If
		End If
	End Function
	...
	Private Function SpellColor(ByRef wname As String, ByVal wclass As String) As String
		...
		'武器名から判定
		If InStr(wname, "ファイヤー") > 0 Or InStr(wname, "フレア") > 0 Or InStr(wname, "ヒート") > 0 Or InStr(wname, "ブラッド") > 0 Then
			SpellColor = "赤"
			Exit Function
		End If
		...
		If InStr(wname, "ウッド") > 0 Or InStr(wname, "フォレスト") > 0 Or InStr(wname, "ポイズン") > 0 Then
			SpellColor = "緑"
			Exit Function
		End If
		
		If InStr(wname, "イビル") > 0 Or InStr(wname, "エビル") > 0 Or InStr(wname, "ダーク") > 0 Or InStr(wname, "デス") > 0 Or InStr(wname, "ナイト") > 0 Or InStr(wname, "シャドウ") > 0 Or InStr(wname, "カース") > 0 Or InStr(wname, "カーズ") > 0 Then
			SpellColor = "黒"
			Exit Function
		End If
		
		If InStr(wname, "アース") > 0 Or InStr(wname, "サンド") > 0 Or InStr(wname, "ロック") > 0 Or InStr(wname, "ストーン") > 0 Then
			SpellColor = "黄"
			Exit Function
		End If
		...
		If InStr(wname, "ホーリー") > 0 Or InStr(wname, "スター") > 0 Or InStr(wname, "ムーン") > 0 Or InStr(wname, "コールド") > 0 Or InStr(wname, "アイス") > 0 Or InStr(wname, "フリーズ") > 0 Then
			SpellColor = "白"
			Exit Function
		End If
	...
	Public Sub NegateEffect(ByRef u As Unit, ByRef t As Unit, ByVal w As Short, ByRef wname As String, ByVal dmg As Integer, ByRef fname As String, ByRef fdata As String, ByVal ecost As Short, ByRef msg As String, ByVal be_quiet As Boolean)
		...
		If LIndex(fdata, 1) = "Ｂ" Or LIndex(fdata, 2) = "Ｂ" Or LIndex(fdata, 3) = "Ｂ" Then
			...
			Else
				If dmg < 0 Then
					DisplaySysMessage(msg & t.Nickname & "の[" & fname & "]が攻撃を吸収した。")
				Else
					DisplaySysMessage(msg & t.Nickname & "の[" & fname & "]が攻撃を防いだ。")
				End If
			End If
		Else
			If Not be_quiet Then
