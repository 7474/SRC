...
Module Help
		...
		Select Case stype
			Case "オーラ"
				If u.FeatureName0("バリア") = "オーラバリア" Then
					msg = "オーラ技「オ」の攻撃力と" & u.FeatureName0("オーラバリア") & "の強度に" & VB6.Format(CInt(100 * slevel)) & "の修正を与える。"
				Else
					msg = "オーラ技「オ」の攻撃力の強度に" & VB6.Format(CInt(100 * slevel)) & "の修正を与える。"
				End If
				If u.IsFeatureAvailable("オーラ変換器") Then
					msg = msg & "また、" & Term("ＨＰ", u) & "、" & Term("ＥＮ", u) & "、" & Term("装甲", u) & "、" & Term("運動性") & "がレベルに合わせてそれぞれ増加する。"
				End If
				
			Case "分身"
				msg = VB6.Format(CInt(100 * slevel \ 16)) & "% の確率で分身し、攻撃を回避する。"
				
			Case "超感覚"
				msg = Term("命中", u) & "・" & Term("回避", u)
				If slevel > 0 Then
					msg = msg & "に +" & VB6.Format(CInt(2 * slevel + 3)) & " の修正を与える。"
				Else
					msg = msg & "に +0 の修正を与える。"
				End If
				If slevel > 3 Then
					msg = msg & ";思念誘導攻撃(サ)の射程を" & VB6.Format(CInt(slevel \ 4)) & "だけ延長する。"
				End If
			...
			Case "知覚強化"
				...
				If slevel > 3 Then
					msg = msg & "思念誘導攻撃(サ)の射程を" & VB6.Format(CInt(slevel \ 4)) & "だけ延長する。"
				End If
				msg = msg & "精神不安定により" & Term("ＳＰ", u) & "消費量が20%増加する"
			...
			Case "切り払い"
				msg = "格闘武器(武)、突進技(突)、実弾攻撃(実)による攻撃を " & VB6.Format(CInt(100 * slevel \ 16)) & "% の確率で切り払って回避する。"
				
			Case "迎撃"
				msg = "実弾攻撃(実)による攻撃を " & VB6.Format(CInt(100 * slevel \ 16)) & "% の確率で迎撃する。"
			...
			Case "Ｓ防御"
				If u.IsFeatureAvailable("盾") Then
					msg = "シールド防御を行い、ダメージを" & VB6.Format(CInt(100 * slevel + 400)) & "減少させる。"
				Else
					msg = VB6.Format(CInt(100 * slevel \ 16)) & "% の確率でシールド防御を行う。"
				End If
			...
			Case "資金獲得"
				...
				ElseIf slevel >= 0 Then 
					msg = msg & "が " & VB6.Format(10 * slevel) & "% 増加する。"
				Else
					msg = msg & "が " & VB6.Format(-10 * slevel) & "% 減少する。"
				End If
			...
			Case "同調率成長"
				If slevel >= 0 Then
					msg = p.SkillName0("同調率") & "の成長率が " & VB6.Format(10 * slevel) & "% 増加する。"
				Else
					msg = p.SkillName0("同調率") & "の成長率が " & VB6.Format(-10 * slevel) & "% 減少する。"
				End If
				
			Case "霊力"
				msg = "現在の" & sname0 & "値にあわせて" & Term("ＨＰ", u) & "・" & Term("ＥＮ", u) & "・" & Term("装甲", u) & "・" & Term("移動力", u) & "を強化する。"
				
			Case "霊力成長"
				If slevel >= 0 Then
					msg = p.SkillName0("霊力") & "の成長率が " & VB6.Format(10 * slevel) & "% 増加する。"
				Else
					msg = p.SkillName0("霊力") & "の成長率が " & VB6.Format(-10 * slevel) & "% 減少する。"
				End If
				
			Case "底力"
				msg = Term("ＨＰ", u) & "が最大" & Term("ＨＰ", u) & "の1/4以下の時に発動。;" & "命中＆回避 +30%、クリティカル発生率 +50%。"
				
			Case "超底力"
				msg = Term("ＨＰ", u) & "が最大" & Term("ＨＰ", u) & "の1/4以下の時に発動。;" & "命中＆回避 +50%、クリティカル発生率 +50%。"
			...
			Case "不屈"
				msg = Term("ＨＰ", u) & "が最大" & Term("ＨＰ", u) & "の1/2以下の時に発動。;" & "損傷率に応じて防御力が増加する。"
				
			Case "素質"
				If Not is_level_specified Then
					msg = "入手する経験値が50%増加する。"
				ElseIf slevel >= 0 Then 
					msg = "入手する経験値が " & VB6.Format(10 * slevel) & "% 増加する。"
				Else
					msg = "入手する経験値が " & VB6.Format(-10 * slevel) & "% 減少する。"
				End If
			...
			Case "再生", "英雄"
				msg = Term("ＨＰ", u) & "が０になった時に" & VB6.Format(CInt(100 * slevel \ 16)) & "%の確率で復活する。"
				
			Case "超能力"
				msg = Term("命中", u) & "・" & Term("回避", u) & "・" & Term("ＣＴ率", u) & "にそれぞれ +5。;" & "サイキック攻撃(超)の攻撃力に +" & VB6.Format(CInt(100 * slevel)) & "。;" & Term("ＳＰ", u) & "消費量を20%削減する。"
			...
			Case "超反応"
				...
				If slevel >= 0 Then
					msg = msg & "にそれぞれ +" & VB6.Format(CInt(2 * slevel)) & " の修正を与える。"
				Else
					msg = msg & "にそれぞれ " & VB6.Format(CInt(2 * slevel)) & " の修正を与える。"
				End If
			...
			Case "術"
				Select Case slevel
					...
					Case Else
						i = 0
				End Select
				msg = "術属性を持つ武装・" & Term("アビリティ", u) & "及び必要技能が" & sname0 & "の武装・" & Term("アビリティ", u) & "の消費" & Term("ＥＮ", u) & "を" & VB6.Format(i) & "%減少させる。"
			...
			Case "技"
				Select Case slevel
					...
					Case Else
						i = 0
				End Select
				msg = "技属性を持つ武装・" & Term("アビリティ", u) & "及び必要技能が" & sname0 & "の武装・" & Term("アビリティ", u) & "の消費" & Term("ＥＮ", u) & "を" & VB6.Format(i) & "%減少させる。"
				
			Case "集中力"
				msg = Term("スペシャルパワー", u) & "の" & Term("ＳＰ", u) & "消費量が元の80%に減少する。"
				
			Case "闘争本能"
				If p.MinMorale > 100 Then
					If Not p.IsSkillLevelSpecified("闘争本能") Then
						msg = "出撃時の" & Term("気力", u) & "が" & VB6.Format(p.MinMorale + 5 * slevel) & "に増加する。"
					ElseIf slevel >= 0 Then 
						msg = "出撃時の" & Term("気力", u) & "が" & VB6.Format(p.MinMorale + 5 * slevel) & "に増加する。"
					Else
						msg = "出撃時の" & Term("気力", u) & "が" & VB6.Format(p.MinMorale + 5 * slevel) & "に減少する。"
					End If
				Else
					If Not p.IsSkillLevelSpecified("闘争本能") Then
						msg = "出撃時の" & Term("気力", u) & "が105に増加する。"
					ElseIf slevel >= 0 Then 
						msg = "出撃時の" & Term("気力", u) & "が" & VB6.Format(100 + 5 * slevel) & "に増加する。"
					Else
						msg = "出撃時の" & Term("気力", u) & "が" & VB6.Format(100 + 5 * slevel) & "に減少する。"
					End If
				End If
			...
			Case "指揮"
				msg = "半径" & StrConv(VB6.Format(p.CommandRange), VbStrConv.Wide) & "マス以内にいる味方ザコ・汎用及び階級所有パイロットの" & Term("命中", u) & "・" & Term("回避", u)
				If slevel >= 0 Then
					msg = msg & "に +" & VB6.Format(CInt(5 * slevel)) & "。"
				Else
					msg = msg & "に " & VB6.Format(CInt(5 * slevel)) & "。"
				End If
			...
			Case "階級"
				If InStr(sname, "階級Lv") = 0 Then
					msg = "階級レベル" & StrConv(VB6.Format(CInt(slevel)), VbStrConv.Wide) & "に相当する。;"
				End If
				msg = msg & "半径" & StrConv(VB6.Format(p.CommandRange), VbStrConv.Wide) & "マス以内にいるザコ及び階級所有パイロットに指揮効果を与える。"
				
			Case "格闘サポート"
				msg = "自分がサポートパイロットの時にメインパイロットの" & Term("格闘", u)
				If slevel >= 0 Then
					msg = msg & "に +" & VB6.Format(CInt(2 * slevel)) & "。"
				Else
					msg = msg & "に " & VB6.Format(CInt(2 * slevel)) & "。"
				End If
			...
			Case "サポート"
				msg = "自分がサポートパイロットの時にメインパイロットの" & Term("命中", u) & "・" & Term("回避", u)
				If slevel >= 0 Then
					msg = msg & "に +" & VB6.Format(CInt(3 * slevel)) & "。"
				Else
					msg = msg & "に " & VB6.Format(CInt(3 * slevel)) & "。"
				End If
				
			Case "広域サポート"
				msg = "半径２マス以内にいる味方パイロットの" & Term("命中", u) & "・" & Term("回避", u)
				If slevel >= 0 Then
					msg = msg & "に +" & VB6.Format(CInt(5 * slevel)) & "。"
				Else
					msg = msg & "に " & VB6.Format(CInt(5 * slevel)) & "。"
			...
			Case "援護"
				msg = "隣接するユニットにサポートアタックとサポートガードを" & "１ターンにそれぞれ" & VB6.Format(CInt(slevel)) & "回行う。"
				
			Case "援護攻撃"
				msg = "隣接するユニットにサポートアタックを１ターンに" & VB6.Format(CInt(slevel)) & "回行う。"
				
			Case "援護防御"
				msg = "隣接するユニットにサポートガードを１ターンに" & VB6.Format(CInt(slevel)) & "回行う。"
				
			Case "統率"
				msg = "自分から攻撃をかけた場合、" & "サポートアタックが同時援護攻撃に変更される。;" & "（１ターンに " & VB6.Format(CInt(slevel)) & "回）"
				
			Case "チーム"
				msg = sdata & "に所属する。" & "同じ" & sdata & "のユニットに対してのみ援護や指揮を行う。"
				
			Case "カウンター"
				msg = "１ターンに " & VB6.Format(CInt(slevel)) & "回" & "反撃がカウンター攻撃になり、相手の攻撃に先制して反撃を行う。"
			...
			Case "先読み"
				msg = VB6.Format(CInt(100 * slevel \ 16)) & "%の確率で" & "反撃がカウンター攻撃になり、相手の攻撃に先制して反撃を行う。"
				
			Case "再攻撃"
				msg = "自分の攻撃の直後に " & VB6.Format(CInt(100 * slevel \ 16)) & "% の確率で再攻撃を行う。" & "ただしパイロットの" & Term("反応", u) & "が相手を下回る場合、確率は半減。"
			...
			Case "耐久"
				If slevel >= 0 Then
					msg = "ダメージ計算の際に" & Term("装甲", u) & "を" & VB6.Format(CInt(5 * slevel)) & "%増加させる。"
				Else
					msg = "ダメージ計算の際に" & Term("装甲", u) & "を" & VB6.Format(CInt(5 * System.Math.Abs(slevel))) & "%減少させる。"
				End If
			...
			Case "ＳＰ回復"
				msg = "毎ターン" & Term("ＳＰ", u) & "がパイロットレベル/8+5回復する(+" & VB6.Format(p.Level \ 8 + 5) & ")。"
				
			Case "格闘成長"
				'攻撃力低成長オプションが指定されているかどうかで解説を変更する。
				msg = "レベルアップ時の" & Term("格闘", u) & "の増加量が"
				If IsOptionDefined("攻撃力低成長") Then
					msg = msg & VB6.Format(slevel + 0.5) & "になる。"
				Else
					msg = msg & VB6.Format(slevel + 1) & "になる。"
				End If
			...
			Case "命中成長"
				msg = "レベルアップ時の" & Term("命中", u) & "の増加量が" & VB6.Format(slevel + 2) & "になる。"
				
			Case "回避成長"
				msg = "レベルアップ時の" & Term("回避", u) & "の増加量が" & VB6.Format(slevel + 2) & "になる。"
				
			Case "技量成長"
				msg = "レベルアップ時の" & Term("技量", u) & "の増加量が" & VB6.Format(slevel + 1) & "になる。"
				
			Case "反応成長"
				msg = "レベルアップ時の" & Term("反応", u) & "の増加量が" & VB6.Format(slevel + 1) & "になる。"
			...
			Case "精神統一"
				msg = Term("ＳＰ", u) & "が最大" & Term("ＳＰ", u) & "の20%未満(" & VB6.Format(p.MaxSP \ 5) & "未満)の場合、" & "ターン開始時に" & Term("ＳＰ", u) & "が最大" & Term("ＳＰ", u) & "の10%分回復する(+" & VB6.Format(p.MaxSP \ 10) & ")。"
				
			Case "損傷時気力増加"
				If slevel >= -1 Then
					msg = "ダメージを受けた際に" & Term("気力", u) & "+" & VB6.Format(CInt(slevel + 1)) & "。"
				Else
					msg = "ダメージを受けた際に" & Term("気力", u) & VB6.Format(CInt(slevel + 1)) & "。"
				End If
			...
			Case "命中時気力増加"
				If slevel >= 0 Then
					msg = "攻撃を命中させた際に" & Term("気力", u) & "+" & VB6.Format(CInt(slevel)) & "。(マップ攻撃は例外)"
				Else
					msg = "攻撃を命中させた際に" & Term("気力", u) & VB6.Format(CInt(slevel)) & "。(マップ攻撃は例外)"
				End If
			...
			Case "失敗時気力増加"
				If slevel >= 0 Then
					msg = "攻撃を外してしまった際に" & Term("気力", u) & "+" & VB6.Format(CInt(slevel)) & "。(マップ攻撃は例外)"
				Else
					msg = "攻撃を外してしまった際に" & Term("気力", u) & VB6.Format(CInt(slevel)) & "。(マップ攻撃は例外)"
				End If
			...
			Case "回避時気力増加"
				If slevel >= 0 Then
					msg = "攻撃を回避した際に" & Term("気力", u) & "+" & VB6.Format(CInt(slevel)) & "。"
				Else
					msg = "攻撃を回避した際に" & Term("気力", u) & VB6.Format(CInt(slevel)) & "。"
				End If
				
			Case "起死回生"
				msg = Term("ＳＰ", u) & "、" & Term("ＨＰ", u) & "、" & Term("ＥＮ", u) & "の全てが最大値の20%以下になると毎ターン最初に発動。" & Term("ＳＰ", u) & "、" & Term("ＨＰ", u) & "、" & Term("ＥＮ", u) & "が全快する。"
				
			Case "戦術"
				msg = "思考パターン決定の際に用いられる" & Term("技量", u)
				If slevel >= 0 Then
					msg = msg & "初期値がレベル×10増加(+" & VB6.Format(CInt(10 * slevel)) & ")。"
				Else
					msg = msg & "初期値がレベル×10減少(" & VB6.Format(CInt(10 * slevel)) & ")。"
				End If
				
			Case "得意技"
				msg = "「" & p.SkillData(stype) & "」属性を持つ武器・" & Term("アビリティ", u) & "によるダメージ・効果量が 20% 増加。" & "また、" & Term("アビリティ", u) & "の継続時間が 40% 増加。"
				
			Case "不得手"
				msg = "「" & p.SkillData(stype) & "」属性を持つ武器・" & Term("アビリティ", u) & "によるダメージ・効果量が 20% 減少。" & "また、" & Term("アビリティ", u) & "の継続時間が 40% 減少。"
			...
			Case "ハンター"
				...
				If slevel >= 0 Then
					msg = msg & VB6.Format(10 * slevel) & "%増加する。"
				Else
					msg = msg & VB6.Format(-10 * slevel) & "%減少する。"
				End If
			...
			Case "スペシャルパワー自動発動"
				msg = Term("気力", u) & "が" & LIndex(sdata, 3) & "以上で発動し、" & "毎ターン最初に「" & LIndex(sdata, 2) & "」が自動でかかる。" & "（" & Term("ＳＰ", u) & "は消費しない）"
				
			Case "修理"
				msg = "修理装置や回復" & Term("アビリティ", u) & "を使った際の" & Term("ＨＰ", u) & "回復量が "
				If slevel >= 0 Then
					msg = msg & VB6.Format(10 * slevel) & "% 増加する。"
				Else
					msg = msg & VB6.Format(-10 * slevel) & "% 減少する。"
			...
			Case "補給"
				...
				End If
				msg = msg & "補給" & Term("アビリティ", u) & "を使った際の" & Term("ＥＮ", u) & "回復量が "
				If slevel >= 0 Then
					msg = msg & VB6.Format(10 * slevel) & "% 増加する。"
				Else
					msg = msg & VB6.Format(-10 * slevel) & "% 減少する。"
			...
			Case "気力上限"
				...
				If slevel <> 0 Then
					i = MaxLng(slevel, 0)
				End If
				msg = Term("気力", u) & "の上限が" & VB6.Format(i) & "になる。"
			...
			Case "気力下限"
				...
				If slevel <> 0 Then
					i = MaxLng(slevel, 0)
				End If
				msg = Term("気力", u) & "の下限が" & VB6.Format(i) & "になる。"
				
				' ADD START MARGE
			Case "遊撃"
				msg = "移動後使用可能な武器・" & Term("アビリティ", u) & "を使った後に、残った移動力を使って移動できる。"
				' ADD END MARGE
	...
	Public Function FeatureHelpMessage(ByRef u As Unit, ByVal findex As Object, ByVal is_additional As Boolean) As String
		...
		Select Case ftype
			Case "シールド"
				...
				msg = sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で防御を行い、" & "ダメージを半減。"
			...
			Case "大型シールド"
				...
				End If
				msg = "(" & sname & "Lv+1)/16の確率(" & VB6.Format(prob) & "%)で防御を行い、" & "ダメージを半減。"
				
			Case "小型シールド"
				sname = p.SkillName0("Ｓ防御")
				prob = p.SkillLevel("Ｓ防御") * 100 \ 16
				msg = sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で防御を行い、" & "ダメージを2/3に減少。"
				
			Case "エネルギーシールド"
				sname = p.SkillName0("Ｓ防御")
				prob = p.SkillLevel("Ｓ防御") * 100 \ 16
				If flevel > 0 Then
					msg = sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で防御を行い、" & "ダメージを半減した上で更に" & VB6.Format(100 * flevel) & "減少。"
				Else
					msg = sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で防御を行い、" & "ダメージを半減。"
				End If
			...
			Case "アクティブシールド"
				...
				End If
				msg = "(" & sname & "Lv+2)/16の確率(" & VB6.Format(prob) & "%)で防御を行い、" & "ダメージを半減。"
			...
			Case "盾"
				...
				End If
				msg = VB6.Format(flevel) & "回、攻撃によって貫通されるまでシールド防御を行い、" & "ダメージを減少させる(-" & VB6.Format(CInt(slevel)) & ")。;" & "ただし攻撃側が「破」属性を持っていた場合、一度に２回分破壊される。;" & "ダメージの減少量はパイロットの" & sname & "レベルによって決まる。"
			...
			Case "バリア"
				If LIndex(fdata, 2) <> "" And LIndex(fdata, 2) <> "全" Then
					...
					End If
				End If
				msg = msg & "ダメージ" & VB6.Format(CInt(1000 * flevel)) & "以下の攻撃を無効化。"
				If IsNumeric(LIndex(fdata, 3)) Then
					If StrToLng(LIndex(fdata, 3)) > 0 Then
						msg = msg & ";発動時に" & LIndex(fdata, 3) & Term("ＥＮ", u) & "消費。"
					ElseIf StrToLng(LIndex(fdata, 3)) < 0 Then 
						msg = msg & ";発動時に" & Mid(LIndex(fdata, 3), 2) & Term("ＥＮ", u) & "増加。"
					End If
				Else
					msg = msg & ";発動時に10ＥＮ消費。"
				End If
				If StrToLng(LIndex(fdata, 4)) > 50 Then
					msg = msg & Term("気力", u) & LIndex(fdata, 4) & "以上で使用可能。"
				End If
					...
					Select Case p.SkillType(opt)
						...
						Case "同調率"
							...
							If u.SyncLevel >= 30 Then
								msg = msg & ";パイロットの" & sname & "により強度が変化(+" & VB6.Format(lv_mod * (u.SyncLevel - 30)) & ")。"
						...
						Case "霊力"
							...
							If lv_mod = -1 Then
								lv_mod = 10
							End If
							msg = msg & ";パイロットの" & sname & "により強度が増加(+" & VB6.Format(lv_mod * u.PlanaLevel) & ")。"
						Case "オーラ"
							sname = p.SkillName0(opt)
							If lv_mod = -1 Then
								lv_mod = 200
							End If
							msg = msg & ";パイロットの" & sname & "により強度が増加(+" & VB6.Format(lv_mod * u.AuraLevel) & ")。"
						Case "超能力"
							sname = p.SkillName0(opt)
							If lv_mod = -1 Then
								lv_mod = 200
							End If
							msg = msg & ";パイロットの" & sname & "により強度が増加(+" & VB6.Format(lv_mod * u.PsychicLevel) & ")。"
						Case Else
							sname = u.SkillName0(opt)
							If lv_mod = -1 Then
								lv_mod = 200
							End If
							msg = msg & ";パイロットの" & sname & "レベルにより強度が増加(+" & VB6.Format(lv_mod * u.SkillLevel(opt)) & ")。"
					End Select
				Next 
			...
			Case "バリアシールド"
				sname = p.SkillName0("Ｓ防御")
				prob = p.SkillLevel("Ｓ防御") * 100 \ 16
				msg = sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で発動し、"
				If LIndex(fdata, 2) <> "" And LIndex(fdata, 2) <> "全" Then
					If Left(LIndex(fdata, 2), 1) = "!" Then
						msg = msg & "「" & Mid(LIndex(fdata, 2), 2) & "」属性を持たない"
					Else
						msg = msg & "「" & LIndex(fdata, 2) & "」属性を持つ"
					End If
				End If
				msg = msg & "ダメージ" & VB6.Format(CInt(1000 * flevel)) & "以下の攻撃を無効化。"
				If IsNumeric(LIndex(fdata, 3)) Then
			...
			Case "広域バリア"
				If IsNumeric(LIndex(fdata, 2)) And LIndex(fdata, 2) <> "1" Then
					msg = "半径" & StrConv(LIndex(fdata, 2), VbStrConv.Wide) & "マス以内の味方ユニットに対する"
				...
				If LIndex(fdata, 3) <> "" And LIndex(fdata, 3) <> "全" Then
					If Left(LIndex(fdata, 3), 1) = "!" Then
						msg = msg & "「" & Mid(LIndex(fdata, 3), 2) & "」属性を持たない"
					Else
						msg = msg & "「" & LIndex(fdata, 3) & "」属性を持つ"
					End If
				End If
				msg = msg & "ダメージ" & VB6.Format(CInt(1000 * flevel)) & "以下の攻撃を無効化。"
				If IsNumeric(LIndex(fdata, 4)) Then
					If StrToLng(LIndex(fdata, 4)) > 0 Then
						msg = msg & ";発動時に" & LIndex(fdata, 4) & Term("ＥＮ", u) & "消費。"
					ElseIf StrToLng(LIndex(fdata, 4)) < 0 Then 
						msg = msg & ";発動時に" & Mid(LIndex(fdata, 4), 2) & Term("ＥＮ", u) & "増加。"
					End If
				Else
					msg = msg & ";発動時に" & VB6.Format(20 * i) & Term("ＥＮ", u) & "消費。"
				End If
				If StrToLng(LIndex(fdata, 5)) > 50 Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 5) & "以上で使用可能。"
				End If
			...
			Case "フィールド"
				...
				End If
				If flevel >= 0 Then
					msg = msg & "攻撃のダメージを" & VB6.Format(CInt(500 * flevel)) & "減少させる。"
				Else
					msg = msg & "攻撃のダメージを" & VB6.Format(CInt(-500 * flevel)) & "増加させる。"
				End If
				If StrToLng(LIndex(fdata, 3)) > 0 Then
					msg = msg & ";発動時に" & LIndex(fdata, 3) & Term("ＥＮ", u) & "消費。"
				ElseIf StrToLng(LIndex(fdata, 3)) < 0 Then 
			...
			Case "プロテクション"
				...
				End If
				If flevel > 10 Then
					msg = msg & "攻撃のダメージを" & VB6.Format(CInt(10 * flevel - 100)) & "%吸収する。"
				ElseIf flevel >= 0 Then 
					msg = msg & "攻撃のダメージを" & VB6.Format(CInt(10 * flevel)) & "%減少させる。"
				Else
					msg = msg & "攻撃のダメージを" & VB6.Format(CInt(-10 * flevel)) & "%増加させる。"
				End If
				If Not IsNumeric(LIndex(fdata, 3)) Then
					msg = msg & ";発動時に10" & Term("ＥＮ", u) & "増加。"
				ElseIf StrToLng(LIndex(fdata, 3)) > 0 Then 
					msg = msg & ";発動時に" & LIndex(fdata, 3) & Term("ＥＮ", u) & "消費。"
				ElseIf StrToLng(LIndex(fdata, 3)) < 0 Then 
					msg = msg & ";発動時に" & Mid(LIndex(fdata, 3), 2) & Term("ＥＮ", u) & "増加。"
			...
			Case "アーマー"
				...
				End If
				If flevel >= 0 Then
					msg = msg & "攻撃に対して装甲を" & VB6.Format(CInt(100 * flevel)) & "増加させる。"
				Else
					msg = msg & "攻撃に対して装甲を" & VB6.Format(CInt(-100 * flevel)) & "減少させる。"
				End If
				If StrToLng(LIndex(fdata, 3)) > 50 Then
					msg = msg & Term("気力", u) & LIndex(fdata, 3) & "以上で使用可能。"
				End If
			...
			Case "レジスト"
				...
				End If
				If flevel > 10 Then
					msg = msg & "攻撃に対してダメージを" & VB6.Format(100 - CInt(10 * flevel)) & "%吸収する。"
				ElseIf flevel >= 0 Then 
					msg = msg & "攻撃に対してダメージを" & VB6.Format(CInt(10 * flevel)) & "%軽減させる。"
				Else
					msg = msg & "攻撃に対してダメージを" & VB6.Format(CInt(-10 * flevel)) & "%増加させる。"
				End If
				If StrToLng(LIndex(fdata, 3)) > 50 Then
					msg = msg & Term("気力", u) & LIndex(fdata, 3) & "以上で使用可能。"
				End If
			...
			Case "当て身技"
				...
				ElseIf InStr(buf, "+") > 0 Or InStr(buf, "-") > 0 Then 
					...
					msg = msg & "(" & sname & "Lv" & Mid(buf, i) & ")/16の確率(" & VB6.Format(prob) & "%)で受け止め、"
				Else
					sname = u.SkillName0(buf)
					prob = u.SkillLevel(buf) * 100 \ 16
					msg = msg & sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で受け止め、"
				End If
				...
				If StrToLng(LIndex(fdata, 5)) > 0 Then
					msg = msg & ";発動時に" & LIndex(fdata, 5) & Term("ＥＮ", u) & "消費。"
				ElseIf StrToLng(LIndex(fdata, 5)) < 0 Then 
					msg = msg & ";発動時に" & Mid(LIndex(fdata, 5), 2) & Term("ＥＮ", u) & "増加。"
				End If
			...
			Case "反射"
				...
				ElseIf InStr(buf, "+") > 0 Or InStr(buf, "-") > 0 Then 
					...
					sname = u.SkillName0(Left(buf, i - 1))
					prob = (u.SkillLevel(Left(buf, i - 1)) + CShort(Mid(buf, i))) * 100 \ 16
					msg = msg & "(" & sname & "Lv" & Mid(buf, i) & ")/16の確率(" & VB6.Format(prob) & "%)で反射。"
				Else
					sname = u.SkillName0(buf)
					prob = u.SkillLevel(buf) * 100 \ 16
					msg = msg & sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で反射。"
				End If
			...
			Case "阻止"
				...
				End If
				If flevel <> 1 Then
					msg = msg & "ダメージ" & VB6.Format(CInt(500 * flevel)) & "以下の"
				End If
				msg = msg & "攻撃を"
				...
				ElseIf InStr(buf, "+") > 0 Or InStr(buf, "-") > 0 Then 
					...
					sname = u.SkillName0(Left(buf, i - 1))
					prob = (u.SkillLevel(Left(buf, i - 1)) + CShort(Mid(buf, i))) * 100 \ 16
					msg = msg & "(" & sname & "Lv" & Mid(buf, i) & ")/16の確率(" & VB6.Format(prob) & "%)で阻止。"
				Else
					sname = u.SkillName0(buf)
					prob = u.SkillLevel(buf) * 100 \ 16
					msg = msg & sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で阻止。"
				End If
			...
			Case "融合"
				prob = flevel * 100 \ 16
				msg = VB6.Format(flevel) & "/16の確率(" & VB6.Format(prob) & "%)で発動し、" & "ダメージを" & Term("ＨＰ", u) & "に変換。;" & "ただし、「武」「突」「接」による攻撃には無効。"
			...
			Case "変換"
				If LIndex(fdata, 2) <> "" And LIndex(fdata, 2) <> "全" Then
					...
					End If
				End If
				msg = msg & "攻撃を受けた際にダメージを" & Term("ＥＮ", u) & "に変換。;" & "変換効率は " & Term("ＥＮ", u) & "増加 ＝ "
			...
			Case "自動反撃"
				...
				ElseIf InStr(buf, "+") > 0 Or InStr(buf, "-") > 0 Then 
					...
					sname = u.SkillName0(Left(buf, i - 1))
					prob = (u.SkillLevel(Left(buf, i - 1)) + CShort(Mid(buf, i))) * 100 \ 16
					msg = msg & "(" & sname & "Lv" & Mid(buf, i) & ")/16の確率(" & VB6.Format(prob) & "%)で、"
				Else
					sname = u.SkillName0(buf)
					prob = u.SkillLevel(buf) * 100 \ 16
					msg = msg & sname & "Lv/16の確率(" & VB6.Format(prob) & "%)で、"
				End If
			...
			Case "ＨＰ回復"
				msg = "毎ターン最大" & Term("ＨＰ", u) & "の" & VB6.Format(10 * flevel) & "%分の" & Term("ＨＰ", u) & "を回復。"
				
			Case "ＥＮ回復"
				msg = "毎ターン最大" & Term("ＥＮ", u) & "の" & VB6.Format(10 * flevel) & "%分の" & Term("ＥＮ", u) & "を回復。"
				
			Case "霊力回復"
				sname = p.SkillName0("霊力")
				msg = "毎ターン最大" & sname & "の" & VB6.Format(10 * flevel) & "%分の" & sname & "を回復。"
				
			Case "ＨＰ消費"
				msg = "毎ターン最大" & Term("ＨＰ", u) & "の" & VB6.Format(10 * flevel) & "%分の" & Term("ＨＰ", u) & "を消費。"
				
			Case "ＥＮ消費"
				msg = "毎ターン最大" & Term("ＥＮ", u) & "の" & VB6.Format(10 * flevel) & "%分の" & Term("ＥＮ", u) & "を消費。"
				
			Case "霊力消費"
				sname = p.SkillName0("霊力")
				msg = "毎ターン最大" & sname & "の" & VB6.Format(10 * flevel) & "%分の" & sname & "を消費。"
				
			Case "分身"
				msg = "50%の確率で攻撃を完全に回避。;" & "発動条件：" & Term("気力", u) & "130以上"
				
			Case "超回避"
				msg = "あらゆる攻撃を" & VB6.Format(10 * flevel) & "%の確率で回避。"
				If IsNumeric(LIndex(fdata, 2)) Then
					If StrToLng(LIndex(fdata, 2)) > 0 Then
						msg = msg & ";発動時に" & LIndex(fdata, 2) & Term("ＥＮ", u) & "消費。"
					ElseIf StrToLng(LIndex(fdata, 2)) < 0 Then 
						msg = msg & ";発動時に" & Mid(LIndex(fdata, 2), 2) & Term("ＥＮ", u) & "増加。"
					End If
				End If
				If StrToLng(LIndex(fdata, 3)) > 50 Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 3) & "以上で使用可能。"
			...
			Case "緊急テレポート"
				msg = "攻撃を受けた際に" & VB6.Format(10 * flevel) & "%の確率で" & "テレポートし、攻撃を回避。;" & "テレポート先は" & LIndex(fdata, 2) & "マス以内の範囲の内、" & "最も敵から遠い地点から選ばれる。"
				If IsNumeric(LIndex(fdata, 3)) Then
					If StrToLng(LIndex(fdata, 3)) > 0 Then
						msg = msg & ";発動時に" & LIndex(fdata, 3) & Term("ＥＮ", u) & "消費。"
					ElseIf StrToLng(LIndex(fdata, 3)) < 0 Then 
			...
			Case "ダミー"
				...
				End If
				msg = buf & "を身代わりにして攻撃を" & VB6.Format(flevel) & "回まで回避。"
			...
			Case "攻撃回避"
				...
				End If
				If flevel >= 0 Then
					msg = msg & "攻撃の命中率を本来の" & VB6.Format(CInt(100 - 10 * flevel)) & "%に減少させる。"
				Else
					msg = msg & "攻撃の命中率を本来の" & VB6.Format(CInt(100 - 10 * flevel)) & "%に増加させる。"
				End If
				If StrToLng(LIndex(fdata, 3)) > 50 Then
					msg = msg & Term("気力", u) & LIndex(fdata, 3) & "以上で使用可能。"
				End If
			...
			Case "抵抗力"
				If flevel >= 0 Then
					msg = "武器の特殊効果を受ける確率を" & VB6.Format(10 * flevel) & "%減少させる。"
				Else
					msg = "武器の特殊効果を受ける確率を" & VB6.Format(-10 * flevel) & "%増加させる。"
				End If
			...
			Case "補給装置"
				msg = "他のユニットの" & Term("ＥＮ", u) & "と弾薬を全快。;" & "ただしユニットのパイロットの" & Term("気力", u) & "は-10。"
				If IsOptionDefined("移動後補給不可") Then
					msg = msg & "移動後は使用不可。"
				End If
			...
			Case "修理不可"
				For i = 2 To CInt(fdata)
					...
					End If
				Next 
				msg = msg & buf & ";ただし、" & Term("スペシャルパワー", u) & "や地形、母艦による回復は可能。"
				
			Case "霊力変換器"
				sname = p.SkillName0("霊力")
				msg = sname & "に合わせて各種能力が上昇する。"
				If flevel_specified Then
					msg = msg & ";（" & sname & "上限 = " & VB6.Format(flevel) & "）"
				End If
			...
			Case "オーラ変換器"
				...
				If flevel_specified Then
					msg = msg & ";（" & sname & "上限レベル = " & VB6.Format(flevel) & "）"
				End If
			...
			Case "ステルス"
				If flevel_specified Then
					msg = "敵から" & StrConv(VB6.Format(flevel), VbStrConv.Wide) & "マス以内にいない限り発見されない。" & "ただし自分から攻撃すると１ターン無効。"
				Else
			...
			Case "テレポート"
				msg = "テレポートを行い、" & Term("移動力", u) & VB6.Format(u.Speed + flevel) & "で地形を無視して移動。;"
			...
			Case "ジャンプ"
				msg = Term("移動力", u) & VB6.Format(u.Speed + flevel) & "で地上地形を無視しながらジャンプ移動。"
				If LLength(fdata) > 1 Then
					If StrToLng(LIndex(fdata, 2)) > 0 Then
						msg = msg & ";" & LIndex(fdata, 2) & Term("ＥＮ", u) & "消費。"
					End If
			...
			Case "ホバー移動"
				msg = "空中に浮きながら移動することで砂漠と雪原の移動コストが１になる。" & "また、水上移動も可能。ただし移動時に5" & Term("ＥＮ", u) & "消費。"
			...
			Case "追加移動力"
				...
				If flevel >= 0 Then
					msg = msg & VB6.Format(flevel) & "増加。"
				Else
					msg = msg & VB6.Format(-flevel) & "減少。"
				End If
			...
			Case "パーツ分離"
				...
				If flevel_specified Then
					msg = msg & ";ユニット破壊時に" & VB6.Format(10 * flevel) & "%の確率で発動。"
				End If
			...
			Case "ハイパーモード"
				...
				If InStr(fdata, "気力発動") > 0 Then
					msg = Term("気力", u) & VB6.Format(100 + 10 * flevel) & "で特殊形態" & uname & "に"
				ElseIf flevel <= 5 Then 
					msg = Term("気力", u) & VB6.Format(100 + 10 * flevel) & "、" & "もしくは" & Term("ＨＰ", u) & "が最大" & Term("ＨＰ", u) & "の1/4以下で特殊形態" & uname & "に"
				Else
					msg = Term("ＨＰ", u) & "が最大" & Term("ＨＰ", u) & "の1/4以下で特殊形態" & uname & "に"
				End If
			...
			Case "合体"
				...
				If LLength(fdata) > 3 Then
					If UDList.IsDefined(LIndex(fdata, 2)) Then
						msg = "以下のユニットと合体し<B>" & UDList.Item(LIndex(fdata, 2)).Nickname & "</B>に" & buf & "; "
					Else
						msg = "以下のユニットと合体し<B>" & LIndex(fdata, 2) & "</B>に" & buf & "; "
					End If
			...
			Case "支配"
				If LLength(fdata) = 2 Then
					If Not PDList.IsDefined(LIndex(fdata, 2)) Then
						ErrorMessage("支配対象のパイロット「" & LIndex(fdata, 2) & "」のデータが定義されていません")
						Exit Function
					End If
					msg = PDList.Item(LIndex(fdata, 2)).Nickname & "の存在を維持し、仕えさせている。"
				Else
					msg = "以下のユニットの存在を維持し、仕えさせている。;"
					For i = 2 To LLength(fdata)
						If Not PDList.IsDefined(LIndex(fdata, 2)) Then
							ErrorMessage("支配対象のパイロット「" & LIndex(fdata, i) & "」のデータが定義されていません")
							Exit Function
						End If
			...
			Case "ＥＣＭ"
				...
				If flevel >= 0 Then
					msg = msg & VB6.Format(100 - 5 * flevel) & "%に減少させる。"
				Else
					msg = msg & VB6.Format(100 - 5 * flevel) & "%に増加させる。"
				End If
				buf = fname
				If InStr(buf, "Lv") Then
					buf = Left(buf, InStr(buf, "Lv") - 1)
			...
			Case "格闘強化"
				If flevel >= 0 Then
					msg = "パイロットの" & Term("格闘", u) & "を+" & VB6.Format(CShort(5 * flevel)) & "。"
				Else
					msg = "パイロットの" & Term("格闘", u) & "を" & VB6.Format(CShort(5 * flevel)) & "。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
			...
			Case "射撃強化"
				If p.HasMana() Then
					If flevel >= 0 Then
						msg = "パイロットの" & Term("魔力", u) & "を+" & VB6.Format(CShort(5 * flevel)) & "。"
					Else
						msg = "パイロットの" & Term("魔力", u) & "を" & VB6.Format(CShort(5 * flevel)) & "。"
					End If
				Else
					If flevel >= 0 Then
						msg = "パイロットの" & Term("射撃", u) & "を+" & VB6.Format(CShort(5 * flevel)) & "。"
					Else
						msg = "パイロットの" & Term("射撃", u) & "を" & VB6.Format(CShort(5 * flevel)) & "。"
					End If
				End If
			...
			Case "命中強化"
				If flevel >= 0 Then
					msg = "パイロットの" & Term("命中", u) & "を+" & VB6.Format(CShort(5 * flevel)) & "。"
				Else
					msg = "パイロットの" & Term("命中", u) & "を" & VB6.Format(CShort(5 * flevel)) & "。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & "気力" & LIndex(fdata, 2) & "以上で発動。"
				End If
				
			Case "回避強化"
				If flevel >= 0 Then
					msg = "パイロットの" & Term("回避", u) & "を+" & VB6.Format(CShort(5 * flevel)) & "。"
				Else
					msg = "パイロットの" & Term("回避", u) & "を" & VB6.Format(CShort(5 * flevel)) & "。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
			...
			Case "技量強化"
				If flevel >= 0 Then
					msg = "パイロットの" & Term("技量", u) & "を+" & VB6.Format(CShort(5 * flevel)) & "。"
				Else
					msg = "パイロットの" & Term("技量", u) & "を" & VB6.Format(CShort(5 * flevel)) & "。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
			...
			Case "反応強化"
				If flevel >= 0 Then
					msg = "パイロットの" & Term("反応", u) & "を+" & VB6.Format(CShort(5 * flevel)) & "。"
				Else
					msg = "パイロットの" & Term("反応", u) & "を" & VB6.Format(CShort(5 * flevel)) & "。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
			...
			Case "ＨＰ強化"
				If flevel >= 0 Then
					msg = "最大" & Term("ＨＰ", u) & "を" & VB6.Format(CShort(200 * flevel)) & "増加。"
				Else
					msg = "最大" & Term("ＨＰ", u) & "を" & VB6.Format(CShort(-200 * flevel)) & "減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "ＥＮ強化"
				If flevel >= 0 Then
					msg = "最大" & Term("ＥＮ", u) & "を" & VB6.Format(CShort(10 * flevel)) & "増加。"
				Else
					msg = "最大" & Term("ＥＮ", u) & "を" & VB6.Format(CShort(-10 * flevel)) & "減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "装甲強化"
				If flevel >= 0 Then
					msg = Term("装甲", u) & "を" & VB6.Format(CShort(100 * flevel)) & "増加。"
				Else
					msg = Term("装甲", u) & "を" & VB6.Format(CShort(-100 * flevel)) & "減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "運動性強化"
				If flevel >= 0 Then
					msg = Term("運動性", u) & "を" & VB6.Format(CShort(5 * flevel)) & "増加。"
				Else
					msg = Term("運動性", u) & "を" & VB6.Format(CShort(-5 * flevel)) & "減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "移動力強化"
				If flevel >= 0 Then
					msg = Term("移動力", u) & "を" & VB6.Format(CShort(flevel)) & "増加。"
				Else
					msg = Term("移動力", u) & "を" & VB6.Format(CShort(flevel)) & "減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "ＨＰ割合強化"
				If flevel >= 0 Then
					msg = "最大" & Term("ＨＰ", u) & "を" & VB6.Format(CShort(5 * flevel)) & "%分増加。"
				Else
					msg = "最大" & Term("ＨＰ", u) & "を" & VB6.Format(CShort(-5 * flevel)) & "%分減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "ＥＮ割合強化"
				If flevel >= 0 Then
					msg = "最大" & Term("ＥＮ", u) & "を" & VB6.Format(CShort(5 * flevel)) & "%分増加。"
				Else
					msg = "最大" & Term("ＥＮ", u) & "を" & VB6.Format(CShort(-5 * flevel)) & "%分減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "装甲割合強化"
				If flevel >= 0 Then
					msg = Term("装甲", u) & "を" & VB6.Format(CShort(5 * flevel)) & "%分増加。"
				Else
					msg = Term("装甲", u) & "を" & VB6.Format(CShort(-5 * flevel)) & "%分減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "運動性割合強化"
				If flevel >= 0 Then
					msg = Term("運動性", u) & "を" & VB6.Format(CShort(5 * flevel)) & "%分増加。"
				Else
					msg = Term("運動性", u) & "を" & VB6.Format(CShort(-5 * flevel)) & "%分減少。"
				End If
				If IsNumeric(LIndex(fdata, 2)) Then
					msg = msg & ";" & Term("気力", u) & LIndex(fdata, 2) & "以上で発動。"
				End If
			...
			Case "ＺＯＣ無効化"
				...
				Else
					msg = "このユニットは敵ユニットによる" & VB6.Format(flevel) & "レベル以下のＺＯＣの影響を受けない。"
				End If
			...
			Case "隣接ユニットＺＯＣ無効化"
				...
				Else
					msg = "このユニットが隣接する敵ユニットによる" & VB6.Format(flevel) & "レベル以下のＺＯＣを無効化する。"
				End If
			...
			Case "広域ＺＯＣ無効化"
				...
				Else
					msg = msg & buf & "マス以内に設定された" & VB6.Format(flevel) & "レベル以下のＺＯＣの影響を無効化する。"
				End If
	...
	Public Function AttributeName(ByRef u As Unit, ByRef atr As String, Optional ByVal is_ability As Boolean = False) As String
		...
		If is_ability Then
			If Right(AttributeName, 2) = "攻撃" Or Right(AttributeName, 2) = "武器" Then
				AttributeName = Left(AttributeName, Len(AttributeName) - 2) & Term("アビリティ", u)
			End If
		End If
	End Function
	...
	Public Function AttributeHelpMessage(ByRef u As Unit, ByRef atr As String, ByVal idx As Short, ByVal is_ability As Boolean) As String
		...
		Select Case atype
			...
			Case "複"
				If p.HasMana() Then
					msg = "格闘と魔法の両方を使った攻撃。" & "パイロットの" & Term("格闘", u) & "と" & Term("魔力", u) & "の" & "平均値を使って攻撃力を算出する。"
				Else
					msg = "格闘と射撃の両方を使った攻撃。" & "パイロットの" & Term("格闘", u) & "と" & Term("射撃", u) & "の" & "平均値を使って攻撃力を算出する。"
				End If
			Case "Ｐ"
			...
			Case "Ｒ"
				...
				Else
					msg = "ユニットランクや特殊能力による攻撃力上昇が" & VB6.Format(10 * alevel) & "％になる。"
				End If
				msg = "ユニットランクや特殊能力による攻撃力上昇が通常の半分。"
			Case "改"
				If alevel = DEFAULT_LEVEL Then
				...
				Else
					msg = "ユニットランクによる攻撃力上昇が" & VB6.Format(10 * alevel) & "％になる。"
				End If
			Case "武"
			...
			Case "体"
				msg = "生命力を攻撃力に換える攻撃。ユニットの" & Term("ＨＰ", u) & "によって攻撃力が変化する。"
			Case "吸"
				msg = "与えたダメージの１／４を吸収し、自分の" & Term("ＨＰ", u) & "に変換。"
			Case "減"
				msg = Term("ＨＰ", u) & "にダメージを与えると同時に相手の" & Term("ＥＮ", u) & "を減少させる。"
			Case "奪"
				msg = Term("ＨＰ", u) & "にダメージを与えると同時に相手の" & Term("ＥＮ", u) & "を減少させ、" & "減少させた" & Term("ＥＮ", u) & "の半分を自分のものにする。"
			Case "貫"
				If alevel > 0 Then
					msg = "相手の" & Term("装甲", u) & "を本来の" & VB6.Format(100 - 10 * alevel) & "％の値とみなしてダメージ計算を行う。"
				Else
			...
			Case "Ａ"
				msg = "使用すると" & VB6.Format(alevel) & "ターン後に再チャージが完了するまで使用不能。"
				If Not is_ability Then
				...
				Else
					...
					If u.IsAbilityClassifiedAs(idx, "共") And u.Ability(idx).Stock = 0 Then
						msg = msg & "同レベルの使用回数共有" & Term("アビリティ", u) & "も連動して使用不能になる。"
					End If
				End If
			Case "合"
				...
				If i > u.CountFeature Then
					ErrorMessage(u.Name & "の合体技「" & waname & "」に対応した合体技能力がありません")
					Exit Function
				End If
			...
			Case "共"
				...
				Else
					...
					If alevel > 0 Then
						msg = msg & ";同レベルの使用回数共有" & Term("アビリティ", u) & "間で使用回数を共有している。"
					End If
				End If
			Case "斉"
				If Not is_ability Then
			...
			Case "気"
				msg = "使用時に気力" & VB6.Format(5 * alevel) & "を消費。"
			Case "霊", "プ"
				msg = "使用時に" & VB6.Format(5 * alevel) & p.SkillName0("霊力") & "を消費。"
			Case "失"
				msg = "使用時に" & VB6.Format(alevel * u.MaxHP \ 10) & "の" & Term("ＨＰ", u) & "を失う。"
			Case "銭"
				msg = "使用時に" & VB6.Format(MaxLng(alevel, 1) * u.Value \ 10) & "の" & Term("資金", u) & "が必要。;" & Term("資金", u) & "が足りない場合は使用不可。"
			Case "消"
				msg = "使用後に1ターン消耗状態に陥り、回避・反撃不能。"
			Case "尽"
				If Not is_ability Then
					If alevel > 0 Then
						msg = "全" & Term("ＥＮ", u) & "を使って攻撃し、使用後に" & Term("ＥＮ", u) & "が0になる。;" & "(残り" & Term("ＥＮ", u) & "－必要" & Term("ＥＮ", u) & ")×" & StrConv(VB6.Format(alevel), VbStrConv.Wide) & "だけ攻撃力が上昇。"
					Else
			...
			Case "除"
				If Not is_ability Then
					msg = "クリティカル発生時に相手にかけられた" & Term("アビリティ", u) & "による特殊効果を打ち消す。"
				Else
					msg = Term("アビリティ", u) & "実行時に、それまでに相手にかけられていた" & Term("アビリティ", u) & "による特殊効果が解除される。"
				End If
			Case "即"
				msg = "クリティカル発生時に相手を即死させる。"
			Case "告"
				If alevel > 0 Then
					msg = "クリティカル発生時に相手を「死の宣告」状態にし、" & StrConv(VB6.Format(CShort(alevel)), VbStrConv.Wide) & "ターン後に" & Term("ＨＰ", u) & "を１にする。"
				Else
			...
			Case "脱"
				...
				ElseIf alevel >= 0 Then 
					msg = "相手の" & Term("気力", u) & "を" & VB6.Format(CShort(5 * alevel)) & "低下させる。"
				Else
					msg = "相手の" & Term("気力", u) & "を" & VB6.Format(CShort(-5 * alevel)) & "増加させる。"
				End If
			Case "Ｄ"
				If alevel = DEFAULT_LEVEL Then
					msg = "相手の" & Term("気力", u) & "を10低下させ、その半分を吸収する。"
				ElseIf alevel >= 0 Then 
					msg = "相手の" & Term("気力", u) & "を" & VB6.Format(CShort(5 * alevel)) & "低下させ、その半分を吸収する。"
				Else
					msg = "相手の" & Term("気力", u) & "を" & VB6.Format(CShort(-5 * alevel)) & "増加させ、その半分を与える。"
				End If
			Case "低攻"
				If alevel = DEFAULT_LEVEL Then
					alevel = 3
			...
			Case "吹"
				If alevel > 0 Then
					msg = "相手ユニットを" & StrConv(VB6.Format(CShort(alevel)), VbStrConv.Wide) & "マス吹き飛ばす。;" & "クリティカル発生時は吹き飛ばし距離＋１。"
				Else
					msg = "クリティカル発生時に相手ユニットを１マス吹き飛ばす。"
				End If
			Case "Ｋ"
				If alevel > 0 Then
					msg = "相手ユニットを" & StrConv(VB6.Format(CShort(alevel)), VbStrConv.Wide) & "マス吹き飛ばす。;" & "クリティカル発生時は吹き飛ばし距離＋１。" & Term("サイズ", u) & "制限あり。"
				Else
					msg = "クリティカル発生時に相手ユニットを１マス吹き飛ばす。" & Term("サイズ", u) & "制限あり。"
				End If
			Case "引"
				msg = "クリティカル発生時に相手ユニットを隣接するマスまで引き寄せる。"
			Case "転"
				msg = "クリティカル発生時に相手ユニットを" & StrConv(VB6.Format(CShort(alevel)), VbStrConv.Wide) & "マス強制テレポートさせる。テレポート先はランダムに選ばれる。"
			Case "連"
				msg = VB6.Format(alevel) & "回連続して攻撃を行う。;" & "攻撃によって与えるダメージは下記の式で計算される。;" & "  通常のダメージ量 × 命中回数 ／ 攻撃回数"
			Case "再"
				msg = VB6.Format(100 * alevel \ 16) & "%の確率で再攻撃。"
			Case "精"
				msg = "精神に働きかける攻撃。性格が「機械」のユニットには効かない。" & "シールドを無効化。"
			Case "援"
				msg = "自分以外のユニットに対してのみ使用可能。"
			Case "難"
				msg = VB6.Format(10 * alevel) & "%の確率で失敗する。"
			Case "忍"
				msg = "物音を立てずに攻撃し、" & "ステルス状態の際に" & Term("ＣＴ率", u) & "に+10のボーナス。" & "一撃で相手を倒した場合は自分から攻撃をかけてもステルス状態が維持される。"
			Case "盗"
				msg = "クリティカル発生時に敵から持ち物を盗む。;" & "盗めるものは通常は" & Term("資金", u) & "(普通に倒した時の半分の額)だが、" & "相手によってはアイテムを入手することもある。"
			Case "Ｈ"
			...
			Case "空"
				...
				End If
			Case "固"
				msg = "パイロットの" & Term("気力", u) & "や攻撃力、防御側の" & Term("装甲", u) & "にかかわらず" & "武器の攻撃力と同じダメージを与える攻撃。" & "ただし、ユニットランクが上がっても攻撃力は増えない。" & Term("スペシャルパワー", u) & "や" & Term("地形適応", u) & "によるダメージ修正は有効。"
			Case "衰"
			...
			Case "写"
				msg = "クリティカル発生時に相手ユニットに変身する。;" & "ただし、既に変身している場合は使用できない。" & "また、相手と２段階以上" & Term("サイズ", u) & "が異なる場合は無効。"
			Case "化"
			...
			Case "対"
				If Not is_ability Then
					whatsthis = "攻撃"
				End If
				msg = "相手のメインパイロットのレベルが" & StrConv(VB6.Format(CShort(alevel)), VbStrConv.Wide) & "の倍数の場合にのみ有効な" & whatsthis & "。"
			Case "ラ"
				If Not is_ability Then
					whatsthis = "攻撃"
				End If
			...
			Case "小"
				msg = "最小射程が" & StrConv(VB6.Format(CShort(alevel)), VbStrConv.Wide) & "になる。"
			Case "散"
